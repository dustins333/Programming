...
import { formatWeight } from "./programming/setLabels";
import { listConditioningLogs, describeConditioningLog } from "./programming/conditioning";
...
export async function listDayTimeline(userId) {
  const [completionsResult, logsResult, nutritionLogs] = await Promise.all([
    programming
      ...
      .select(
        `id, completed_at, group_workout_id, spc_workout_id, one_off_workout_id, alternate_session_id,
         ...
         alternate_sessions ( title, alternate_programs ( name ) )`
      )
      .eq("user_id", userId)
    ...
    programming
      .from("logs")
      .select("exercise_id, date_performed, group_workout_id, spc_workout_id, one_off_workout_id, alternate_session_id")
      .eq("user_id", userId)
  ...
  // Conditioning (0130) — isolated the same way. A member who isn't on
  // conditioning simply has none.
  let conditioningLogs = [];
  try {
    conditioningLogs = await listConditioningLogs(userId, { limit: 200 });
  } catch {
    // leave empty
  }
  ...
  const sessionKeyOf = (row) =>
    row.group_workout_id
      ? `g:${row.group_workout_id}`
      : row.spc_workout_id
        ? `s:${row.spc_workout_id}`
        : row.one_off_workout_id
          ? `o:${row.one_off_workout_id}`
          : row.alternate_session_id
            ? // Alternate sessions repeat weekly off one row, so the key
              // needs the date too or every week of a run would tally into
              // one bucket. The completion side keys the same way.
              `a:${row.alternate_session_id}:${row.date_performed}`
            : null;
  ...
  const sessionEntries = completionsResult.data.map((c) => {
    ...
    } else if (c.alternate_sessions) {
      // The run's own name leads, because that's what she saw on My Week
      // ("Italy trip"), and the session title says which of the two or
      // three she did.
      const runName = c.alternate_sessions.alternate_programs?.name;
      label = runName ? `${runName} — ${c.alternate_sessions.title}` : c.alternate_sessions.title;
    } else {
      label = c.one_off_workouts?.title || "One-off workout";
    }
    const counts =
      sessionCounts.get(
        c.alternate_session_id ? `a:${c.alternate_session_id}:${date}` : sessionKeyOf(c)
      ) ?? dateCounts.get(date);
    return {
      type: "session",
      id: c.id,
  ...
  const nutritionEntries = nutritionLogs
    ...
    .map((log) => {
      ...
      if (formatWeight(log.weight)) parts.push(formatWeight(log.weight));
      return {
        type: "nutrition",
        id: log.id,
        date: log.date,
  ...
  const conditioningEntries = conditioningLogs.map((log) => ({
    type: "conditioning",
    id: log.id,
    date: log.performed_on,
    label: "Conditioning",
    subtitle: describeConditioningLog(log, { separator: " · " }),
    // The row itself, so the popup it opens needs no second fetch.
    log,
  }));

  return [...sessionEntries, ...nutritionEntries, ...milestoneEntries, ...conditioningEntries].sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 0));
}
