...
// reopen it to fix anything after) — worth a real pause before it fires,
// same web/native branch as the rest of this file.
export function confirmFinalizePayroll(periodLabel) {
  const message = `Finalize your entries for ${periodLabel}? You won't be able to add or edit anything for this period afterward unless an admin reopens it.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
