...
// SPC builder's warm-up "Copy to...". Names the sessions that already have a
// warm-up, since those are the ones that lose something.
export function confirmReplaceWarmups(names) {
  const list = names.length === 1 ? names[0] : `${names.slice(0, -1).join(", ")} and ${names[names.length - 1]}`;
  const message = `${list} already ${names.length === 1 ? "has a warm-up" : "have warm-ups"}. Replace ${names.length === 1 ? "it" : "them"} with this one?`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Replace warm-ups?", message, [
      { text: "Cancel", style: "cancel", onPress: () => resolve(false) },
...
// The SPC counterpart of the above. Writes status 'inactive' (0108), which is
// "not an SPC client at all" rather than the deliberate hold that 'paused' is,
// so she leaves the SPC roster and the live board's picker. Nothing is
// destroyed — the spc_clients row is kept precisely because spc_blocks
// cascades off it, so every program she has ever been written survives and
// comes back the moment she is switched on again.
export function confirmTurnSpcOff(name) {
  const message = `Turn off SPC for ${name}? They'll come off the SPC roster and the live board, and stop being counted as an SPC client. Their programs and history are kept, and you can turn SPC back on here.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Turn off SPC?", message, [
      { text: "Cancel", style: "cancel", onPress: () => resolve(false) },
      { text: "Turn off", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
...
// file.
// A member deleting a photo she attached to one of her own lifts. Its own
// wording rather than reusing confirmDeletePhoto, whose message talks about
// comparisons and check-in history — that's the progress-photo flow, and none
// of it is true here. Same named-variant reasoning as confirmDeleteAnnouncement
// sitting alongside the block-specific confirmDelete.
export function confirmDeleteLiftPhoto() {
  const message = "Delete this photo? It will be removed from this lift's history. This can't be undone.";
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
...
// Leaving an order page with things in the bag that haven't been sent. The
// bag is kept either way (lib/programming/eventBag.js), so the message says
// so: this is a reminder, not a threat of losing anything.
export function confirmLeaveUnsentBag(count) {
  const items = count === 1 ? "1 item" : `${count} items`;
  const message = `You have ${items} in your bag that haven't been sent yet. Your bag is saved, but nothing is ordered until you submit.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(`${message}\n\nLeave anyway?`));
  }
  return new Promise((resolve) => {
    Alert.alert("Leave without submitting?", message, [
...
// Cancelling a program she published for a future Monday (SPC simplification).
// Deliberately not worded as a delete, because it isn't one: the sessions go
// back to the build space exactly as they were, which is the same place a
// replaced-before-it-started program lands. Naming the client matters here —
// the whole risk is cancelling the wrong person's.
export function confirmCancelQueuedProgram(clientName, startLabel) {
  const who = clientName || "this client";
  const message =
    `This program was going live to ${who} on ${startLabel}. Cancelling takes it off her schedule and puts it back in your build space as a draft.` +
    "\n\nNothing is lost: every session, lift and note stays exactly as you built it, and you can publish it again for any Monday.";
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Cancel this program?", message, [
      { text: "Keep it scheduled", style: "cancel", onPress: () => resolve(false) },
      { text: "Cancel program", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
}

// Deleting a category never deletes the templates filed under it (the FK is
// `on delete set null`, 0110), so the count is stated to make that concrete
// rather than leaving a coach to guess whether five workouts are about to
// disappear.
export function confirmDeleteTemplateCategory(name, count) {
  const tail =
    count > 0
      ? ` The ${count} ${count === 1 ? "template" : "templates"} in it will stay in the library as uncategorised.`
      : "";
  const message = `Delete the category "${name}"?${tail}`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Delete this category?", message, [
      { text: "Cancel", style: "cancel", onPress: () => resolve(false) },
      { text: "Delete", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
...
// Ending a run early takes it off the member's phone straight away, which
// is the point (she's home), but it is worth saying out loud.
export function confirmEndAlternateProgram(name) {
  const message = `End "${name}" now? It comes off their My Week straight away, and their normal sessions start counting again from today. Anything they already logged is kept.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("End this now?", message, [
      { text: "Cancel", style: "cancel", onPress: () => resolve(false) },
      { text: "End it", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
}

export function confirmDeleteAlternateProgram(name) {
  const message = `Delete "${name}" completely? This removes the assignment and anything logged against it. To stop it early but keep the record, use "End now" instead.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Delete this assignment?", message, [
      { text: "Cancel", style: "cancel", onPress: () => resolve(false) },
      { text: "Delete", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
...
// Reopening a session a coach (or the member) already finalized. Deliberately
// gated where marking one finalized is a single tap: finalizing on her behalf
// is correcting a forgotten tap, reopening is undoing something she said she
// had finished. Mirrors the two-step un-finalize on the live board.
export function confirmUnfinalizeSession(label) {
  const message = `${label} is finalized. Reopening it takes it back off her record until it's finalized again.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
    Alert.alert("Reopen this session?", message, [
      { text: "Keep it finalized", style: "cancel", onPress: () => resolve(false) },
      { text: "Reopen", style: "destructive", onPress: () => resolve(true) },
    ]);
  });
}

// Deleting a benchmark takes every member's logged entries with it (the
// entries cascade), so this names what goes rather than asking a generic
// "are you sure" — a coach tidying up an old quarter should know she is
// throwing away everyone's numbers from it, not just a row of dates.
export function confirmDeleteBenchmark(name) {
  const message = `Delete the ${name} benchmark? Every member's kettlebells and notes from it go too. This can't be undone.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
