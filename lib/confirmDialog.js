...
export function confirmPublishEvent(title, audienceLabel, goLiveAt = null) {
  const message = goLiveAt
    ? `Schedule "${title}" for ${formatDateTimeInBoise(goLiveAt)}? Nobody sees it until then — the Events tab, the popup and the notification all arrive together at that time.`
    : `Publish "${title}" to ${audienceLabel}? It appears on their Events tab right away, and stays there until it closes.`;
  if (Platform.OS === "web") {
    return Promise.resolve(window.confirm(message));
  }
  return new Promise((resolve) => {
