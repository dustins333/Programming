...
// "05:00" → "5:00 AM". Postgres hands a `time` column back with seconds, the
// option list doesn't have them, so the value is trimmed before lookup.
export function formatTimeLabel(value) {
  if (!value) return "";
  const hm = String(value).slice(0, 5);
  return TIME_OPTIONS.find((o) => o.value === hm)?.label ?? hm;
}
