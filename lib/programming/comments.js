...
// Notes for several blocks at once. The SPC client page's History tab puts a
// note count on every finished run's meta line, not just the one that happens
// to be open, so the lazy per-run fetch the sessions expansion uses would be
// one query per row to render a collapsed list.
//
// Same two-step as listComments — names merged client-side rather than
// through a cross-schema embed. Returns a Map so a run with no notes is a
// miss, not a row to filter out.
export async function listCommentsForBlocks({ spcBlockIds }) {
  const ids = [...new Set((spcBlockIds ?? []).filter(Boolean))];
  ...
  for (const c of comments) {
    if (!byBlock.has(c.spc_block_id)) byBlock.set(c.spc_block_id, []);
    byBlock.get(c.spc_block_id).push({ ...c, coachName: nameById[c.coach_id] ?? "Coach" });
  }
  return byBlock;
}
