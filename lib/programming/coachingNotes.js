...
// An SPC block has one spc_workouts row per (block, week, session) since 0016,
// so the workout id alone identifies the week — no week_number comparison
// needed, matching what the hub board already does.
function isSameSession(note, session) {
  if (session?.groupWorkoutId) return note.group_workout_id === session.groupWorkoutId;
  if (session?.spcWorkoutId) return note.spc_workout_id === session.spcWorkoutId;
