...
export async function addTemplateExercise({ templateId, exerciseId, position }) {
  const { data, error } = await programming
    .from("template_exercises")
    .insert({ template_id: templateId, exercise_id: exerciseId, position, sets: 3, reps: "10" })
    .select("*, exercises(id, name, muscle_group, movement_pattern, video_url)")
    .single();
  if (error) throw error;
  return data;
...
export async function reorderTemplateExercises(items) {
  await Promise.all(items.map((item) => programming.from("template_exercises").update({ position: item.position }).eq("id", item.id)));
}
