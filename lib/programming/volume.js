import { countsAsVolume } from "./repUnit";
...
// Carries and holds are the other half of the inflation, and no threshold
// reaches them: a Farmer Carry at "60 reps @ 62 lb" is 3,720 lb from 60
// seconds of walking, which sits squarely inside the range real heavy sets
// occupy. reps x weight is only arithmetic when the reps are reps, so
// anything measured in seconds/feet/meters contributes 0 — see
// lib/programming/repUnit.js.
...
//
// `exercise` is the row's own { tracks_weight, rep_unit } — pass the embed
// straight through. Missing means "caller couldn't tell" and is treated as
// weighted reps, so a forgotten embed can't silently zero a real lift.
export function setVolume(reps, weight, exercise) {
  if (exercise?.tracks_weight === false) return 0;
  if (!countsAsVolume(exercise)) return 0;
...
export function sumVolume(rows) {
  let total = 0;
  for (const row of rows ?? []) total += setVolume(row.reps, row.weight, row.exercises);
