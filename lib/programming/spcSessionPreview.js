...
import { getSpcClient } from "./spcClients";
...
export async function getSpcSessionPreview(userId, today = todayInBoise()) {
  ...
  const [detail, goal, spcClient] = await Promise.all([
    ...
    // KEEP IN MIND (spc_clients.notes_goals_feedback). Read here rather than
    // off the caller's client object so the sheet carries it whichever way it
    // was opened — the roster row has it, the review deck's clients are only
    // {userId, name, sessionNumber}, and a screen that shows a coach's notes
    // on one route and silently not on another is worse than not showing them.
    getSpcClient(userId).catch(() => null),
  ]);
  ...
  return {
    ...
    goal: goal?.goal ?? null,
    keepInMind: spcClient?.notes_goals_feedback ?? null,
    sessions,
  };
}
