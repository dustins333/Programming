...
export async function getExerciseStats(userId, today = todayInBoise()) {
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, exercises(id, name, muscle_group, tracks_weight)")
    .eq("user_id", userId)
...
export async function countPersonalRecordsOn(date = todayInBoise()) {
  ...
  const { data, error } = await programming
    .from("logs")
    .select("user_id, exercise_id, date_performed, reps, weight, exercises(id, tracks_weight)")
    .in("user_id", userIds)
...
export async function getSessionBests(userId, exerciseIds, date) {
  ...
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, exercises(id, name, tracks_weight)")
    .eq("user_id", userId)
    .in("exercise_id", ids)
    .lte("date_performed", date)
    .order("date_performed", { ascending: true })
