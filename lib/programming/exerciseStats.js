...
import { isRampUpSet, externalLoad } from "./setLabels";
...
//
// Ramp-up sets (0116) are excluded. In practice a ramp-up is lighter than the
// working sets by definition, so it would rarely win this anyway — but
// "rarely" is not "never", and the one time it did (a member who ramped
// heavy and then dropped the weight when it moved badly) it would announce a
// PR for the set she specifically told us not to count.
export function topSetOf(rows) {
  const real = rows.filter((r) => !isRampUpSet(r) && (r.reps != null || r.weight != null));
...
export async function getExerciseStats(userId, today = todayInBoise()) {
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, set_type, exercises(id, name, muscle_group, tracks_weight, rep_unit)")
    .eq("user_id", userId)
  ...
  for (const [exerciseId, { exercise, byDate }] of grouped) {
    ...
    for (const [date, rows] of byDate) {
      ...
      if (topSet) sessions.push({ date, weight: externalLoad(topSet), reps: topSet.reps ?? null });
    }
...
export async function countPersonalRecordsOn(date = todayInBoise()) {
  ...
  const { data, error } = await programming
    .from("logs")
    .select("user_id, exercise_id, date_performed, reps, weight, set_type, exercises(id, tracks_weight, rep_unit)")
    .in("user_id", userIds)
  ...
  for (const byDate of byPair.values()) {
    ...
    for (const [sessionDate, rows] of byDate) {
      ...
      if (top) sessions.push({ date: sessionDate, weight: externalLoad(top) });
    }
...
export async function getSessionBests(userId, exerciseIds, date) {
  ...
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, set_type, exercises(id, name, tracks_weight, rep_unit)")
    .eq("user_id", userId)
    .in("exercise_id", ids)
    .lte("date_performed", date)
    .order("date_performed", { ascending: true })
  ...
  for (const exerciseId of ids) {
    ...
    for (const [sessionDate, rows] of entry.byDate) {
      const topSet = topSetOf(rows);
      if (topSet) sessions.push({ date: sessionDate, weight: externalLoad(topSet), reps: topSet.reps ?? null });
    }
    if (sessions.length < PR_MIN_SESSIONS) continue;
