...
// SPC's dashboard rows and its roster share one set of status keys
// (lib/programming/spcStatus.js), so no mapping is needed here.
export function spcRosterRoute(status) {
  return `/(coach)/spc?status=${status}`;
}
