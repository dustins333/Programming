...
export function programWeekCount(program) {
  ...
  return Math.min(program.weeks, Math.floor(daysBetween(end, program.start_date) / 7) + 1);
}
...
export async function endAlternateProgram(id, endDate = todayInBoise()) {
  const { error } = await programming.from("alternate_programs").update({ ended_at: endDate }).eq("id", id);
  if (error) throw error;
}
