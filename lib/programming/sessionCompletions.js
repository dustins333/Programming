...
// THE COMPLETED-AT RULE, shared by all four finalize functions below.
//
// An explicit date always wins — the "log a missed session" flows and the
// coach logging on a client's behalf pass one, and the bubble and history
// have to reflect when she actually trained, not when it got typed in.
//
// NO date means "now if this is new, and LEAVE IT ALONE if it already
// exists". Re-stamping an existing row is a real data-visibility bug, not a
// cosmetic one: the member's logging screen decides which day's sets to show
// from this timestamp (see plan.js's datePerformed), so tapping Finalize
// again on a session trained days ago repointed it at today and her sets
// silently vanished from her own screen. Hit for real 2026-09-05 — a client
// re-finalized a session she had done three days earlier, and spent the
// morning looking at empty boxes with 19 logged sets sitting behind them.
//
// Un-finalizing DELETES the row, so re-doing a session properly (un-finalize,
// log, finalize) still gets today's date. Only a second tap of Finalize on a
// row that is already there is protected.
export async function finalizeGroupSession(userId, groupWorkoutId, completedAt = null) {
  const existing = await getGroupCompletion(userId, groupWorkoutId);
  if (existing) {
    if (!completedAt) return existing;
  ...
  const { data, error } = await programming
    .from("session_completions")
    .insert({ user_id: userId, group_workout_id: groupWorkoutId, completed_at: stamp })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// See finalizeGroupSession for the completed-at rule — same in all four.
...
export async function finalizeSpcSession(userId, spcWorkoutId, completedAt = null, { instance = null } = {}) {
  const stamp = completedAt ?? new Date().toISOString();
  // The week still resolves against the stamp — an existing row is found by
  // the week it is IN, which is the week the caller is looking at.
  const weekNumber = await spcCompletionWeek(spcWorkoutId, stamp);
  ...
  const { data, error } = await programming
    .from("session_completions")
    .insert({ user_id: userId, spc_workout_id: spcWorkoutId, week_number: weekNumber, instance: instance ?? 1, completed_at: stamp })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// `completedAt` is the completion's own timestamp, for reopening a session in
// a PAST week — without it spcCompletionWeek resolves against today and this
// silently finds nothing (or the wrong week's row) for anything but the
// current one. Deliberately a date rather than a week number: the week still
// comes out of spcCompletionWeek, so the rule that no caller ever supplies a
// raw week holds. Omitted, behaviour is exactly as before.
export async function unfinalizeSpcSession(userId, spcWorkoutId, { instance = null, completedAt = null } = {}) {
  const weekNumber = await spcCompletionWeek(spcWorkoutId, completedAt);
  let target = instance;
  if (target == null) {
...
// Same rule (see finalizeGroupSession). No caller backdates a one-off, so
// there is no date parameter at all — but a second tap must still not move it.
export async function finalizeOneOffSession(userId, oneOffWorkoutId) {
  ...
  const { data, error } = await programming
    .from("session_completions")
    .insert({ user_id: userId, one_off_workout_id: oneOffWorkoutId, completed_at: new Date().toISOString() })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// The board's Finalize is a toggle, so a one-off needs a way back too. A
// one-off completion is keyed on the workout alone (0008), so there is only
// ever the one row to remove.
export async function unfinalizeOneOffSession(userId, oneOffWorkoutId) {
  const { error } = await programming
    .from("session_completions")
    .delete()
    .eq("user_id", userId)
...
// Map<oneOffWorkoutId, completed_at> for one client, so the coach's SPC page
// can split her one-offs into open (Sessions tab) and finished (History).
export async function listOneOffCompletionDetails(userId) {
  const { data, error } = await programming
    .from("session_completions")
    ...
    .not("one_off_workout_id", "is", null);
  if (error) throw error;
  return new Map((data ?? []).map((r) => [r.one_off_workout_id, r.completed_at]));
}
