...
// An SPC completion is keyed on the workout's AUTHORED week (0007's unique
// index), which since 0101 is not necessarily the week the session is being
// shown in — a session can be moved with spc_workouts.scheduled_week. Every
// function below therefore reads the authored week off the row itself rather
// than taking it as a parameter: a caller that passed the *displayed* week
// would write a completion nothing else could ever find, and there is no way
// to tell that has happened from the outside.
async function authoredWeekFor(spcWorkoutId) {
  const { data, error } = await programming.from("spc_workouts").select("week_number").eq("id", spcWorkoutId).maybeSingle();
  if (error) throw error;
...
export async function getSpcCompletion(userId, spcWorkoutId) {
  const weekNumber = await authoredWeekFor(spcWorkoutId);
  const { data, error } = await programming
    .from("session_completions")
    .select("*")
    .eq("user_id", userId)
...
export async function finalizeSpcSession(userId, spcWorkoutId, completedAt = new Date().toISOString()) {
  const weekNumber = await authoredWeekFor(spcWorkoutId);
  const existing = await getSpcCompletion(userId, spcWorkoutId);
  if (existing) {
    const { error } = await programming.from("session_completions").update({ completed_at: completedAt }).eq("id", existing.id);
    if (error) throw error;
    return { ...existing, completed_at: completedAt };
...
export async function unfinalizeSpcSession(userId, spcWorkoutId) {
  const weekNumber = await authoredWeekFor(spcWorkoutId);
  const { error } = await programming
    .from("session_completions")
    .delete()
