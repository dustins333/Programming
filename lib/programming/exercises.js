...
export async function getExercise(id) {
  const { data, error } = await programming.from("exercises").select("id, name, tracks_weight, type").eq("id", id).maybeSingle();
  if (error) throw error;
  return data;
}
...
// cleared them when switching type. parentExerciseId is also lift-only —
// grouping variations under a parent movement doesn't apply to warm-ups.
...
export async function createExercise({ name, type = "lift", muscleGroups, movementPatterns, parentExerciseId, defaultSets, defaultReps, tracksWeight, cues, videoUrl, createdBy }) {
  ...
  const { data, error } = await programming
    ...
    .insert({
      ...
      // on the default rather than carrying a meaningless false.
...
export async function updateExercise(id, { name, type = "lift", muscleGroups, movementPatterns, parentExerciseId, defaultSets, defaultReps, tracksWeight, cues, videoUrl }) {
  const isWarmup = type === "warmup";
  const { data, error } = await programming
    .from("exercises")
    .update({
