...
export async function createExercise({ name, type = "lift", muscleGroups, movementPatterns, parentExerciseId, defaultSets, defaultReps, repUnit, tracksWeight, cues, videoUrl, createdBy, approved = false }) {
...
export async function updateExercise(id, { name, type = "lift", muscleGroups, movementPatterns, parentExerciseId, defaultSets, defaultReps, repUnit, tracksWeight, cues, videoUrl }) {
  const isWarmup = type === "warmup";
  const { data, error } = await programming
    .from("exercises")
    .update({
