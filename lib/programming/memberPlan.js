...
export async function logResult({ userId, exerciseId, datePerformed, setNumber, reps, weight, notes, source, session }) {
  ...
  if (existing) {
    ...
    // A real upsert, not a plain insert. Two autosaves racing — two devices,
    // the gym display and a phone, a fast double-write — both reach here
    // having each found no existing row, and both insert the same set. That
    // silently created duplicates (20 groups of them by Aug 2026), which
    // top-set/PR logic survives but anything that SUMS does not: session
    // volume and set counts were inflated wherever it happened.
    //
    // `logs_unique_set_idx` now backs this. The conflict target must name the
    // index's columns exactly. It is NULLS NOT DISTINCT, which is what makes
    // the ordinary Kova case work at all — every session column is null for a
    // row with no session reference, and plain UNIQUE would treat each of
    // those as distinct and let the duplicates straight back through.
    //
    // The session columns are IN the key on purpose, so two genuinely
    // different sessions on one date stay separate rows; so is
    // truecoach_import_id, because linking two TrueCoach lifts to the same
    // Kova exercise is a supported thing members do (0066) and each link
    // materialises its own set rows.
    const { error } = await programming.from("logs").upsert(
      {
        user_id: userId,
      ...
      {
        onConflict:
          "user_id,exercise_id,date_performed,set_number,group_workout_id,spc_workout_id,one_off_workout_id,truecoach_import_id,week_number",
      }
    );
    if (error) throw error;
  }
