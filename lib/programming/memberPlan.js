...
//
// Returns Map<exerciseId, { date, topSet }> where topSet is that session's
// heaviest logged set (ties broken by reps), skipping any date whose rows
// are all blank — same "a date with rows but no data isn't a last time"
...
export async function listLastLoggedSessions(userId, exerciseIds, today) {
  ...
  for (const [exerciseId, byDate] of byExercise) {
    for (const [date, sets] of byDate) {
      ...
      result.set(exerciseId, { date, topSet });
