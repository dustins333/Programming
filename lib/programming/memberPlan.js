...
// cross-schema core.users lookups elsewhere in this module. Ordered by
// set_number after date so the first row seen per exercise is that date's
// set 1 (a representative "top set"), not an arbitrary later set — used by
// My History's "By Workout" view to show each exercise's last reps/weight
// alongside its last-logged date.
export async function listLoggedExercises(userId) {
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, reps, weight, exercises(id, name, muscle_group)")
    .eq("user_id", userId)
    .order("date_performed", { ascending: false })
    .order("set_number", { ascending: true });
  if (error) throw error;
  ...
  for (const row of data) {
    if (!seen.has(row.exercise_id)) {
      seen.set(row.exercise_id, { exercise: row.exercises, lastDate: row.date_performed, lastReps: row.reps, lastWeight: row.weight });
    }
  }
  return [...seen.values()];
}

// All logged sets for one calendar date, across every exercise — backs My
// History's "By Day" session popup (tapping a session row shows what was
// actually logged that date, since session_completions doesn't itself
// store per-exercise/per-set data).
