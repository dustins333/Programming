...
import { workingSets } from "./setLabels";
...
export function sessionColumns(session) {
  return {
    ...
    // Alternate programming (0110). Deliberately does NOT set week_number:
    // a log row is already keyed by date_performed, and the date says which
    // week of the run it belongs to. Storing the week as well would be a
    // second answer that can disagree with the first.
    alternate_session_id: session?.alternateSessionId ?? null,
  };
}
...
function withSessionFilter(query, cols) {
  let q = query;
  for (const col of ["group_workout_id", "spc_workout_id", "one_off_workout_id", "alternate_session_id"]) {
    q = cols[col] === null ? q.is(col, null) : q.eq(col, cols[col]);
  }
  return q;
}

const NO_SESSION = { group_workout_id: null, spc_workout_id: null, one_off_workout_id: null, alternate_session_id: null };

export async function logResult({ userId, exerciseId, datePerformed, setNumber, reps, weight, notes, setType, source, session }) {
  const setNum = setNumber ?? 1;
  const cols = sessionColumns(session);
  const hasSession = Boolean(
    cols.group_workout_id || cols.spc_workout_id || cols.one_off_workout_id || cols.alternate_session_id
  );
  ...
  const base = () =>
    programming
      ...
      // Never adopt or overwrite a row that came from a TrueCoach import
      // (0066). Those carry a truecoach_import_id and are deleted wholesale
      // on unlink — if a back-logged Kova set landed on one of them (same
      // lift, same date, same set number), unlinking would silently take
      // her Kova set with it. Imported rows are read-only history; her own
      // logging always gets its own row.
  ...
  // `notes` is only written when the caller actually mentions it. The lift
  // note moved out of logs.notes and into exercise_coaching_notes in 0087, so
  // the loggers no longer pass one — and a blanket `notes: notes ?? null`
  // here would have had every autosave quietly erase whatever note was
  // already on the row from before the move. (BacklogSetGrid never passed one
  // either, so it was doing exactly that already.) A caller that DOES pass
  // notes still gets the old behaviour, null included, since clearing a field
  // you had filled in is a real edit.
  if (notes !== undefined) fields.notes = notes ?? null;
  // Same rule for set_type (0116), and for a sharper reason than notes: the
  // logging card is not the only thing that writes a set. The wall display
  // and the back-log grid both come through here without an opinion about
  // whether a set is a ramp-up, and a blanket `set_type: setType ?? 'working'`
  // would have a coach saving a note at the rack silently promote her ramp-up
  // back into a working set. Only a caller that actually says so changes it.
  if (setType !== undefined) fields.set_type = setType;
  ...
  if (existing) {
    ...
    // Two autosaves racing — two devices, the gym display and a phone, a
    // fast double-write — both reach here having each found no existing
    // row, and both insert the same set. That silently created duplicates
    // (20 groups of them by Aug 2026), which top-set/PR logic survives but
    // anything that SUMS does not: session volume and set counts were
    // inflated wherever it happened. `logs_unique_set_idx` is what stops
    // it, and this handles the loser of the race.
    //
    // Deliberately a plain insert with a caught unique violation, NOT an
    // ON CONFLICT upsert. An upsert has to name the index's columns
    // exactly, which quietly couples this function to the index's shape:
    // widening that index (as 0110/0112 does, to keep two away sessions on
    // one day apart) makes every already-loaded browser tab fail with
    // 42P10 on its next new set, because the tab is still naming the old
    // column list. That broke production for real while 0110 was being
    // written. Nothing here names a column list any more, so the index can
    // change again without a coordinated deploy.
    const row = {
      user_id: userId,
      exercise_id: exerciseId,
      date_performed: datePerformed,
      set_number: setNum,
      truecoach_import_id: null,
      ...cols,
      ...fields,
    };
    const { error } = await programming.from("logs").insert(row);
    // 23505 means the other writer got there first. Re-read with the same
    // session-scoped lookup and update what they wrote, which is exactly
    // what the upsert did. Any other error is real and propagates.
    if (error && error.code === "23505") {
      const { data: raced, error: raceError } = await (
        hasSession ? withSessionFilter(base(), cols) : withSessionFilter(base(), NO_SESSION)
      ).maybeSingle();
      if (raceError) throw raceError;
      if (raced) {
        const { error: updateError } = await programming.from("logs").update(fields).eq("id", raced.id);
        if (updateError) throw updateError;
      }
      // If the row that blocked us isn't findable by our own session-scoped
      // lookup, it belongs to a different session and must not be touched.
      // Nothing to do: the set the member typed is genuinely unwritable
      // until the index is widened, which is what 0112 is for.
    } else if (error) {
      throw error;
    }
  }
...
// Remove one set outright. Only reachable from the "−" that replaces the
// "+" at the end of a lift's last row when that row is empty (an
// accidentally added set), so it never destroys anything a member typed —
// but a set she typed into and then cleared has already been written as a
// row of nulls, and without this that husk brings the deleted row straight
// back on the next load.
//
// Scoped exactly like logResult's own lookup: session-scoped first, then
// the session-less fallback for pre-0063 rows, and never a TrueCoach import
// (0066), which is read-only history that belongs to the import, not to
// this session.
export async function deleteLoggedSet({ userId, exerciseId, datePerformed, setNumber, session }) {
  const setNum = setNumber ?? 1;
  const cols = sessionColumns(session);
  const hasSession = Boolean(
    cols.group_workout_id || cols.spc_workout_id || cols.one_off_workout_id || cols.alternate_session_id
  ...
  const { error } = await withSessionFilter(base(), hasSession ? cols : NO_SESSION);
  if (error) throw error;
  // A session-scoped delete only matches rows already stamped with this
  // session, so the session-less fallback runs too — same reasoning as
  // logResult's adopt path. Safe because a session-less row carrying real
  // data would have been read back into this card by getLoggedSetsForDate's
  // own fallback, which would have made the row full and hidden the "−".
  if (!hasSession) return;
  const { error: legacyError } = await withSessionFilter(base(), NO_SESSION);
...
export async function getLoggedSetsForDate(userId, exerciseId, date, session) {
  ...
  const base = () =>
    programming
      ...
      // Imported TrueCoach rows are history, not "her sets for this session" —
      // without this a back-logged date that already holds imported sets
      // would open the logger pre-filled with TrueCoach numbers.
      .is("truecoach_import_id", null)
...
export async function listLastLoggedSessions(userId, exerciseIds, today) {
  ...
  const { data, error } = await programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, set_type")
    .eq("user_id", userId)
  ...
  for (const [exerciseId, byDate] of byExercise) {
    for (const [date, sets] of byDate) {
      ...
      // Ramp-ups stay in `sets` — the Last time sheet shows them, marked,
      // because knowing she ramped into it last week is worth seeing. They
      // are kept OUT of the top set, which is the one number progression is
      // judged on. A day whose only rows are ramp-ups has no top set, and a
      // "last time" line built from one would be misleading rather than
      // merely thin, so that day is passed over for the one before it.
      const working = workingSets(real);
...
// Which exercises a session programs, and in what order the coach wrote
// them — the piece programming.logs cannot supply.
//
// A log row records (user, exercise, date, set) and carries no position, so
// anything reconstructed from logs alone comes back ordered by exercise_id.
// That is a uuid, i.e. effectively random: a coach reading back a finalized
// session saw its lifts shuffled, with no clue that the order on screen
// wasn't the order they were programmed or performed in. Position lives on
// the workout's own exercise rows, so a reader has to ask for it.
//
// superset_group_id comes along because the same rows are what
// sessionLabels.js letters from (A, B1+B2, C) — one query, both facts.
//
// Returns null when the session key is empty, which callers read as "leave
// the logs in whatever order they arrived" rather than "show nothing".
const SESSION_EXERCISE_SOURCES = [
  { key: "group_workout_id", table: "group_workout_exercises" },
  { key: "spc_workout_id", table: "spc_workout_exercises" },
  { key: "one_off_workout_id", table: "one_off_exercises" },
  { key: "alternate_session_id", table: "alternate_exercises" },
];

export async function listSessionExercisePlan(session) {
  const cols = sessionColumns(session);
  const source = SESSION_EXERCISE_SOURCES.find((s) => cols[s.key]);
  if (!source) return null;

  const { data, error } = await programming
    .from(source.table)
    .select("id, exercise_id, position, superset_group_id")
    .eq(source.key, cols[source.key])
    // Position has genuinely collided across rows in the past (before
    // nextPosition landed), and a tie left to Postgres comes back in
    // whatever order it feels like — so the id tiebreak is what keeps this
    // stable between two reads of the same session.
...
export async function listLogsForSession(userId, session) {
  const cols = sessionColumns(session);
  if (!cols.group_workout_id && !cols.spc_workout_id && !cols.one_off_workout_id && !cols.alternate_session_id)
    return null;
