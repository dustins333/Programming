...
// Whether this event has already had an announcement made for it, in any
// channel. Used to default both channel checkboxes off on a re-publish, so
// taking an event down and putting it back doesn't notify everyone twice.
// Counted rather than read off events.pushed_at, which only ever records a
// real push and so would miss an in-app-only or still-scheduled one.
export async function countAnnouncementsForEvent(eventId) {
