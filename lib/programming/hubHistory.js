  ...
  return sessions
    .map((s) => ({
      ...
      loggedSets: s.clients.reduce(
        (sum, c) => sum + (counts.get(setsKey(s.date, c.user_id, c.one_off_workout_id ?? c.group_workout_id ?? c.spc_workout_id)) ?? 0),
        0
      ),
    }))
...
async function countLoggedSets(sessions) {
  ...
  for (const s of sessions) {
    for (const c of s.clients) {
      ...
      if (c.one_off_workout_id) oneOffIds.add(c.one_off_workout_id);
      else if (c.group_workout_id) groupIds.add(c.group_workout_id);
  ...
  const base = () =>
    programming
      .from("logs")
      .select("user_id, date_performed, spc_workout_id, group_workout_id, one_off_workout_id")
  ...
  if (oneOffIds.size > 0) queries.push(base().in("one_off_workout_id", [...oneOffIds]));
