...
// The columns that make up a lift or warm-up row's content, i.e. everything
// except its id and which session it belongs to. Every group copy goes
// through these so a new column only has to be added once. tempo and rest
// used to be missing from the inline list copyWorkoutContent carried, so
// every duplicated week silently lost its rest times (the member app's rest
// timer reads `rest`).
export function exerciseCopyFields(e) {
  return {
...
export function warmupCopyFields(w) {
  return {
...
export async function copyWorkoutContent(sourceWorkoutId, targetWorkoutId) {
  ...
  if (warmups.length > 0) {
    const { error } = await programming.from("group_workout_warmups").insert(
      warmups.map((w) => ({ group_workout_id: targetWorkoutId, ...warmupCopyFields(w) }))
    );
    if (error) throw error;
  }
  ...
  if (exercises.length > 0) {
    const { error } = await programming.from("group_workout_exercises").insert(
      exercises.map((e) => ({ group_workout_id: targetWorkoutId, ...exerciseCopyFields(e) }))
    );
    if (error) throw error;
  }
}
...
export async function listWarmupsForWorkouts(workoutIds) {
  ...
  const { data, error } = await programming
    .from("group_workout_warmups")
    .select("id, group_workout_id, position, label, sets, reps, superset_group_id, exercises(id, name)")
    .in("group_workout_id", workoutIds)
    .order("position");
  if (error) throw error;
  const byWorkout = {};
