...
// column.
//
// This is the whole prefill mechanism, and it depends on a member being able
// to READ a finished benchmark's event row. 0120 gated that read to the
// event's own window, which meant this returned null by the time the next
// benchmark came round and Last time silently came up blank; 0121 widened it
// to any event whose show_from has passed. If Last time ever stops
// prefilling, check that policy first — her entries are not the fragile part,
// they have no window on them at all.
//
// Returns null for a genuine first benchmark, which is correct: she types
// what she remembers into the Last column and that override lives on the
// CURRENT event.
...
// Did she place a bell on anything at all?
//
// `completedMovements` above answers "what has she confirmed", which is what
// the progress count and the results button are measured against. This
// answers the different question the results screens ask: was she here.
// Anyone who got even one movement in showed up, so she keeps her confetti
// and her card to share; the "Don't miss the next one!" treatment is for
// someone who missed the day outright.
//
// Keyed on `tier` rather than `completedAt` on purpose — a bell placed and
// then never confirmed still means she was in the gym. `canComplete` requires
// a tier, so this is always a superset of `completedMovements`.
export function loggedMovements(board) {
  return MOVEMENTS.filter((m) => board?.[m]?.tier);
}

export function missedBenchmark(board) {
  return loggedMovements(board).length === 0;
}
