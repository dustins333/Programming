...
// The line under the number on the celebration, and it is deliberately not
// there for every result.
//
// A tier gain and a variation move both get NOTHING: the pill above already
// says "Up 1 tier" or names the move, and a sentence of encouragement on top
// of a number that speaks for itself reads as padding. The two that remain
// are the two doing real work — a held tier could otherwise read as failure,
// and a first entry has nothing to compare against at all.
export function celebrationCopy(tone) {
  if (tone === "grad" || tone === "up") return "";
  if (tone === "flat") return "Same tier, and you held it. That counts.";
  return "On the board. Now you have something to chase.";
}
