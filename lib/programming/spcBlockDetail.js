...
import { STARTED_SET_FLOOR, isLoggedSet } from "./spcSessionActivity";
import { isRampUpSet } from "./setLabels";
...
export async function getSpcBlockDetail(userId, blockId, today = todayInBoise()) {
  ...
  const buildSession = (w, week) => {
    ...
    const real = logs.filter(isLoggedSet);
    const loggedSets = real.length;
    // The day she actually trained, whether or not she finalized. loggedDate
    // is completion-derived and stays null for an unfinalized session, so
    // without this the read-out has no date to match PRs against and shows
    // none for exactly the sessions a coach is now able to open.
    const lastLoggedDate = real.reduce((latest, r) => (!latest || r.date_performed > latest ? r.date_performed : latest), null);
    ...
    return {
      ...
      state: sessionState({ workout, block, completedAt, liftCount: lifts.length, loggedSets, today }),
    };
  };
...
function sessionState({ workout, block, completedAt, liftCount, loggedSets = 0, today }) {
  ...
  // She trained and didn't tap Finalize. Ranked above the date-based states
  // deliberately: a past week with 16 sets in it is not "skipped", and this
  // used to be the only thing the coach side had to say about it.
...
function summarize(sessions, block, today) {
  ...
  const started = sessions.filter((s) => s.state === "started");
  ...
  // track or it isn't.
  //
  // A started session counts as trained. The sets are the evidence she was
  // in the gym; Finalize is her confirmation, not the event. Counting it
  // against her would be reporting a forgotten tap as a missed workout.
  const due = logged.length + started.length + skipped.length;
  ...
  return {
    ...
    adherence: due > 0 ? Math.round(((logged.length + started.length) / due) * 100) : null,
    setsSkipped,
...
async function listBlockLogs(userId, start, end) {
  let query = programming
    .from("logs")
    .select("exercise_id, date_performed, set_number, reps, weight, notes, set_type, spc_workout_id, exercises(id, name)")
    .eq("user_id", userId)
...
export function buildSessionReadout({ session, logsByDate, personalRecords = [] }) {
  ...
  for (const pr of personalRecords) {
    if (pr.date === (session.loggedDate ?? session.lastLoggedDate)) prByExercise.set(pr.exerciseId, pr);
  }
  ...
  const rows = session.lifts.map((lift) => {
    ...
    // Working sets and ramp-ups are separated BEFORE anything is paired, and
    // that is a bug fix, not just support for a new flag. Every target used
    // to be matched against sets[i] by raw position, so a ramp-up sitting at
    // position 1 shifted the whole lift by one: set 1's target was judged
    // against the warm-up she had already told us not to count, and the
    // "N short" verdict a coach reads was wrong for the entire lift.
    const working = sets.filter((s) => !isRampUpSet(s));
    const ramps = sets.filter(isRampUpSet);
    setsCompleted += working.filter((s) => s.reps != null || s.weight != null).length;

    let shortfall = 0;
    // One row per prescribed set, plus any extra working set she added past
    // the prescription — those used to fall off the read-out entirely.
    const workingRows = Array.from({ length: Math.max(targets.length, working.length) }, (_, i) => {
      const target = targets[i] ?? null;
      const actual = working[i] ?? null;
      ...
      return {
        ...
        setNumber: i + 1,
        rampUp: false,
        // Synthetic rows for a prescribed set she never logged have no stored
        // position, so they sort to the end rather than in among real ones.
        order: actual?.set_number ?? Number.MAX_SAFE_INTEGER,
        target,
        reps,
        weight,
        met,
        logged: reps != null || weight != null,
      };
    });

    // Ramp-ups are shown (a coach wants to know she warmed into it) and
    // counted nowhere: no target, no volume, no set count, no verdict.
    const rampRows = ramps.map((s, i) => ({
      key: `r${i}`,
      setNumber: i + 1,
      rampUp: true,
      order: s.set_number ?? 0,
      target: null,
      reps: s.reps ?? null,
      weight: s.weight ?? null,
      met: true,
      logged: s.reps != null || s.weight != null,
    }));

    // Back into the order she actually performed them.
    const setRows = [...workingRows, ...rampRows].sort((a, b) => a.order - b.order);
    ...
    return {
      ...
      result: pr
        ...
        : shortfall > 0
          ? "short"
          : workingRows.some((r) => r.logged)
            ? "as_written"
            : "not_logged",
    };
  });

  // Working sets only — the denominator of "14 of 18 sets" is what was asked
  // for, and nobody asked for a ramp-up.
  const totalSets = rows.reduce((n, r) => n + r.sets.filter((s) => !s.rampUp).length, 0);
  return {
    rows,
    setsCompleted,
    totalSets,
