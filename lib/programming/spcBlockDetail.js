...
export async function getSpcBlockDetail(userId, blockId, today = todayInBoise()) {
  ...
  const { data: noteRows, error: notesError } = await programming
    ...
    .select("exercise_id, spc_workout_id, body, author_name, created_at")
    .eq("user_id", userId)
    .in("spc_workout_id", workoutIds.length ? workoutIds : [NO_ROWS_UUID])
    .order("created_at", { ascending: false });
  if (notesError) throw notesError;

  // Newest per (workout, lift) wins — the table is append-only, so a later
  // note supersedes rather than replacing in place.
  const notesByWorkout = new Map();
  for (const row of noteRows ?? []) {
    if (!notesByWorkout.has(row.spc_workout_id)) notesByWorkout.set(row.spc_workout_id, new Map());
    const forWorkout = notesByWorkout.get(row.spc_workout_id);
    if (!forWorkout.has(row.exercise_id)) forWorkout.set(row.exercise_id, row);
  }
  ...
  const sessions = workouts.map((w) => {
    ...
    return {
      ...
      notesByExerciseId: notesByWorkout.get(w.id) ?? new Map(),
