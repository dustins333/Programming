...
// inventing a task.
function describeCoverage({ status, current, daysLeft, coverage, nextQueued, draftBlock, notes }) {
  if (status === "paused") {
    return { reason: notes?.trim() || "Paused", nextStep: NEXT_STEP.resume };
  }
  // A draft that's been written but never sent is the most actionable thing
  // this client has, whatever else is or isn't running — it beats every
  // "build the next one" prompt below, because the next one already exists.
  if (draftBlock && !nextQueued) {
    if (!current) return { reason: "Draft written, not sent", nextStep: NEXT_STEP.send };
    if (daysLeft != null && daysLeft <= 7) {
      const when = daysLeft < 0 ? "Block ended" : `${daysLeft} day${daysLeft === 1 ? "" : "s"} left`;
      return { reason: `${when}, draft not sent`, nextStep: NEXT_STEP.send };
    }
  }
  if (!current) {
