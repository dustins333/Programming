...
// Which GoHighLevel calendar this coach's nutrition clients book their
// weekly check-in Zoom calls onto (0077). Null falls back to the gym-wide
// nutrition_checkin_calendar_id setting — that fallback is deliberate, not
// a gap, since a client can have no coach assigned at all
// (public.clients.coach_id is nullable, 0033).
export async function updateCoachCalendar(userId, calendarId) {
  const { error } = await core.from("users").update({ ghl_calendar_id: calendarId ?? null }).eq("id", userId);
  if (error) throw error;
}

// Which of the SPC/Nutrition/Library-Reviewer modules a coach account can
