...
export function findDuplicateCandidates(exercises, dismissedKeys = new Set()) {
  const rows = exercises
    ...
    .map((e) => {
      ...
      return { exercise: e, norm, tokens: [...new Set(norm.split(" "))].sort().join(" ") };
    })
...
export async function mergeExercises(retireId, keepId) {
  ...
  // Variations that hung off the retired entry follow it across, otherwise
  // they'd point at an archived parent and disappear from the library's
  // grouped list.
  const { error: parentError } = await programming
    .from("exercises")
    .update({ parent_exercise_id: keepId })
    .eq("parent_exercise_id", retireId);
