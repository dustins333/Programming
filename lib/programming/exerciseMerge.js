...
export async function mergeExercises(retireId, keepId) {
  ...
  // A member's own lift photos move too, and they need the RPC rather than a
  // plain update in the loop above. programming.lift_photos is owner-only
  // with no staff policy at all (0126), so a coach running this merge matches
  // ZERO rows and gets NO error -- measured, not assumed. Her photo would
  // silently stay attached to the half that then gets archived while her sets
  // moved to the survivor, and it would just stop appearing in her history.
  //
  // Deliberately NOT added to REFERENCE_TABLES: that list also drives
  // getExerciseUsageCount, which would read zero for the same reason and
  // quietly understate what archiving an exercise affects.
  const { error: photoError } = await programming.rpc("repoint_lift_photos", {
    from_exercise: retireId,
    to_exercise: keepId,
  });
  if (photoError) throw new Error(`Couldn't move lift photos: ${photoError.message}`);
