...
export async function addStagedClient(stagedId, { userId, name, sessionNumber, newInstance = false }, existing = []) {
  ...
  const { error } = await programming.from("hub_staged_clients").insert({
    ...
    // "She has already done this one this week, and is doing it again."
    // Recorded rather than acted on: the second completion is opened when the
    // group actually starts (hub_start_staged → hub_open_makeup, 0117), so a
    // group staged across a block-week boundary can't file one against a week
    // she has logged nothing in.
    new_instance: Boolean(newInstance),
    position,
  });
  if (error) throw error;
}
...
export async function resolveStagedSession(id, onDate = null) {
  ...
  return (data ?? []).map((r) => ({
    ...
    reason: r.reason,
    newInstance: Boolean(r.new_instance),
...
// Save the whole group at once: whoever the coach ended up with, minus
// whoever they took off. Used by the hub's Stage tab, where the picker holds
// the entire selection rather than adding clients one at a time.
//
// Diffed rather than cleared-and-rewritten, so a client who was already on
// the group keeps her slot (position) instead of being renumbered by an edit
// that didn't touch her — the board renders slots in position order, and
// people stand at the racks in that order.
export async function syncStagedClients(stagedId, slots, existing = []) {
  const wanted = new Map(slots.map((s) => [s.userId, s]));
  const kept = [];
  for (const row of existing) {
    const want = wanted.get(row.user_id);
    if (!want) {
      await removeStagedClient(stagedId, row.user_id);
      continue;
    }
    // Same person, different session (or a changed make-up answer): the
    // position is hers, only the fields change.
    const patch = {};
    if (want.sessionNumber != null && want.sessionNumber !== row.session_number) {
      patch.session_number = want.sessionNumber;
    }
    if (Boolean(want.newInstance) !== Boolean(row.new_instance)) {
      patch.new_instance = Boolean(want.newInstance);
    }
  ...
  for (const want of wanted.values()) {
    await addStagedClient(
      stagedId,
      { userId: want.userId, name: want.name, sessionNumber: want.sessionNumber, newInstance: want.newInstance },
      kept
    );
    // Mirror the slot addStagedClient just took — lowest free, not next in
    // sequence — or two additions in one save would both be handed the same
    // position and the second would collide.
    const taken = new Set(kept.map((c) => c.position));
    let position = 1;
    while (taken.has(position) && position <= 4) position += 1;
    kept.push({ position });
  }
