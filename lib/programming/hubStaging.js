...
export async function syncStagedClients(stagedId, slots, existing = []) {
  ...
  for (const row of existing) {
    ...
    // Same person, different session: the position is hers, only the number
    // changes.
    if (want.sessionNumber != null && want.sessionNumber !== row.session_number) {
      const { error } = await programming
        .from("hub_staged_clients")
        .update({ session_number: want.sessionNumber })
        .eq("staged_session_id", stagedId)
  ...
  for (const want of wanted.values()) {
    await addStagedClient(stagedId, { userId: want.userId, name: want.name, sessionNumber: want.sessionNumber }, kept);
