...
export function buildResponsesCsv(event, responses, items, questions) {
  ...
  if (event.response_type === "order") {
    const header = ["Member", "Email", "Item", "Option", "Qty", "Price", "Line total", "Submitted", ...questionTexts];
      ...
      for (const li of response.lineItems ?? []) {
        lines.push(
          [
            ...
            li.option ?? "",
            li.qty,
            itemsById[li.event_item_id]?.price ?? "",
            itemsById[li.event_item_id]?.price != null
              ? (Number(itemsById[li.event_item_id].price) * Number(li.qty)).toFixed(2)
              : "",
