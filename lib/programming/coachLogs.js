...
import { dateInBoise, todayInBoise, addDays, dayOfWeekInBoise, boiseInstantFrom } from "../boiseDate";
import { isLoggedSet, sessionActivityState } from "./spcSessionActivity";
...
export function describeCompletion(c) {
  if (c.group_workouts) {
  ...
  if (c.spc_workouts) {
    // The COMPLETION's week wins. 0016 gave SPC one row per (block, week,
    // session), which made the workout row authoritative — and 0105 reversed
    // it: a sessions-format run has one row spanning every week, authored at
    // week 1, so trusting the row labelled every session "Week 1". It was
    // doing that to 8 of the 38 live SPC completions. session_completions
    // .week_number is resolved by spcCompletionWeek and is right under both
    // models; the row is now only the fallback for a row that has no week.
    const week = c.week_number ?? c.spc_workouts.week_number;
    return {
...
//
// The pre-0063 heuristic, now only a FALLBACK. Logs written before that
// migration (and the ~830 rows its backfill couldn't attribute) carry no
// session, so those sessions still have to be narrowed this way. Its old
// limitation still applies to exactly those rows: an exercise programmed in
// two sessions completed the same day appears under both, because the log
// genuinely can't say which. Anything logged from here on is filtered by
// real session id instead and doesn't have the problem.
// Returns null when the session has no exercise rows at all, which the
// caller reads as "don't filter" rather than "show nothing".
// The completion's session identity in the shape logs now stores (0063), so
// a caller can ask for exactly this session's sets.
export function sessionKeyForCompletion(c) {
  if (c.group_workout_id) return { groupWorkoutId: c.group_workout_id };
  if (c.spc_workout_id) return { spcWorkoutId: c.spc_workout_id, weekNumber: c.week_number };
  if (c.one_off_workout_id) return { oneOffWorkoutId: c.one_off_workout_id };
  return null;
}
...
export async function listSessionsSinceAllUsers(sinceDate, limit = 500) {
  ...
  const { data, error } = await programming
    ...
    .select(
      `id, user_id, completed_at, week_number, group_workout_id, spc_workout_id, one_off_workout_id,
  ...
  const rows = data
    .map((c) => ({
      ...
      // Deliberately no exerciseIds here, unlike listRecentSessionsForUser —
      // embedding every workout's exercise list across the whole roster is
      // far too heavy for a feed. Before 0063 that meant this feed's
      // expansion showed the entire calendar day unfiltered; the session key
      // is three plain columns and fixes it at no cost.
  ...
  // A finalized session is not the only session that happened. Sets are
  // stamped with their session from the first keystroke (0063), so a client
  // who trained and never tapped Finalize is fully recorded and was simply
  // never counted — this feed said she did nothing. Isolated: the started
  // half failing leaves the finalized feed exactly as it was.
  const started = await listStartedSessionsSince(sinceDate, rows).catch(() => []);
  const all = [...rows, ...started].sort((a, b) => (a.completedAt < b.completedAt ? 1 : -1));
  ...
  return all.map((r) => ({ ...r, userName: nameById[r.userId] ?? "Unknown" }));
}

// Sessions with sets against them and no completion row, for the same window.
//
// Grouped by (client, session, day) rather than by day alone: two sessions on
// one date are two entries, which is exactly what 0063 exists to make
// possible. Rows with no session reference are skipped rather than guessed
// at — attributing them by date is the old bug where three sessions finalized
// in one evening each showed the others' lifts.
async function listStartedSessionsSince(sinceDate, finalized) {
  const { data, error } = await programming
    .from("logs")
    .select("user_id, date_performed, week_number, group_workout_id, spc_workout_id, one_off_workout_id, reps, weight")
    .gte("date_performed", sinceDate)
    .limit(5000);
  if (error) throw error;

  // Keyed without the date: a session finalized days after it was logged is
  // still finalized, and must not also appear here as started.
  const settled = new Set(finalized.map((r) => `${r.userId}|${keyOf(r.session)}`));

  const groups = new Map();
  for (const row of data ?? []) {
    if (!isLoggedSet(row)) continue;
    const session = sessionKeyForCompletion(row);
    if (!session) continue;
    const settleKey = `${row.user_id}|${keyOf(session)}`;
    if (settled.has(settleKey)) continue;
    const key = `${settleKey}|${row.date_performed}`;
    const g = groups.get(key) ?? { userId: row.user_id, session, date: row.date_performed, loggedSets: 0 };
    g.loggedSets += 1;
    groups.set(key, g);
  }

  const candidates = [...groups.values()].filter((g) => sessionActivityState({ loggedSets: g.loggedSets }) === "started");
  if (candidates.length === 0) return [];

  // Titles in one query per kind, not per row: embedding the workout on every
  // set would repeat it fifteen times a session across the whole roster.
  const ids = (k) => [...new Set(candidates.map((c) => c.session[k]).filter(Boolean))];
  const [groupW, spcW, oneOffW] = await Promise.all([
    fetchLabels("group_workouts", "session_number, week_number, title, group_blocks ( group_programs ( name ) )", ids("groupWorkoutId")),
    fetchLabels("spc_workouts", "session_number, week_number, title", ids("spcWorkoutId")),
    fetchLabels("one_off_workouts", "title", ids("oneOffWorkoutId")),
  ]);

  return candidates.flatMap((c) => {
    const shaped = {
      week_number: c.session.weekNumber ?? null,
      group_workouts: groupW.get(c.session.groupWorkoutId) ?? null,
      spc_workouts: spcW.get(c.session.spcWorkoutId) ?? null,
      one_off_workouts: oneOffW.get(c.session.oneOffWorkoutId) ?? null,
    };
    // describeCompletion falls through to "One-off" when nothing resolves, so
    // a workout that has since been deleted would be listed under the wrong
    // kind. Dropped instead — there is nothing honest to call it.
    if (!shaped.group_workouts && !shaped.spc_workouts && !shaped.one_off_workouts) return [];
    return {
      id: `started:${c.userId}:${keyOf(c.session)}:${c.date}`,
      userId: c.userId,
      date: c.date,
      // Noon Boise on the day she trained, so this sorts against real
      // completion timestamps without a bare date reading as the previous
      // evening once it goes through dateInBoise.
      completedAt: boiseInstantFrom(c.date, "12:00"),
      ...describeCompletion(shaped),
      session: c.session,
      finalized: false,
      loggedSets: c.loggedSets,
    };
  });
}
...
function keyOf(session) {
  ...
  if (session.groupWorkoutId) return `g:${session.groupWorkoutId}`;
  if (session.spcWorkoutId) return `s:${session.spcWorkoutId}:${session.weekNumber ?? ""}`;
  return `o:${session.oneOffWorkoutId}`;
}

async function fetchLabels(table, columns, ids) {
  if (!ids.length) return new Map();
  const { data, error } = await programming.from(table).select(`id, ${columns}`).in("id", ids);
  if (error) throw error;
...
export async function listRecentSessionsForUser(userId, limit = 12) {
  const { data, error } = await programming
    .from("session_completions")
    .select(
      `id, completed_at, week_number, group_workout_id, spc_workout_id, one_off_workout_id,
  ...
  return data.map((c) => ({
    ...
    completedAt: c.completed_at,
    ...describeCompletion(c),
    // The real session key (0063) — lets a reader pull exactly this
    // session's logs instead of everything logged that calendar day.
    session: sessionKeyForCompletion(c),
    // Retained as the fallback for logs written before 0063, which carry no
    // session at all. See RecentSessionsCard.
