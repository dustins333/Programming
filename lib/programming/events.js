...
import { todayInBoise, addDays, boiseInstantFrom } from "../boiseDate";
...
// publishAt is when members actually start seeing it — null means right
// now, which is the whole of the previous behaviour. Member-facing RLS gates
// on the column (migration 0096), so a scheduled event is genuinely hidden
// rather than just filtered client-side.
export async function publishEvent(eventId, publishAt = null) {
...
export async function addEventItem(
  eventId,
  { name, description = null, options = [], position = 0, imagePath = null, price = null }
) {
  const trimmed = String(name || "").trim();
  if (!trimmed) throw new Error("Item name is required");
  const { data, error } = await programming
    .from("event_items")
    .insert({ event_id: eventId, name: trimmed, description, options, position, image_path: imagePath, price })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// An order item's options (flavors, sizes), normalized. They started as
// plain strings in the options jsonb; each can now be an object carrying its
// own photo, { name, image_path }. Strings still read fine, so no migration:
// an older item simply has options with no photo. `name` is what orders
// store in event_response_items.option, so it's the key everywhere.
export function itemOptions(item) {
  return (item?.options ?? [])
    .map((opt) =>
      typeof opt === "string"
        ? { name: opt, image_path: null }
        : { name: String(opt?.name ?? ""), image_path: opt?.image_path ?? null }
    )
    .filter((opt) => opt.name);
}

// Price is dollars per unit, or null (0129). "$30" rather than "$30.00" for a
// whole-dollar price; cents only when there are cents.
...
// A pasted "$34.99" or "34.99" -> 34.99; blank -> null; anything else ->
// undefined, so a caller can tell "cleared" from "not a price".
...
// Copies an event into a new DRAFT, items (with their photos and prices) and
// questions included, so a recurring order ("Supplements", every other month)
// is one tap plus new dates rather than a rebuild.
//
// Everything that describes what went OUT is left behind: status, publish
// schedule and pushed_at, so the copy can never look already announced.
// Dates are kept when they're still ahead and reset otherwise, since copying
// a closed event's closes_at would make a draft that's already over. Image
// paths are shared rather than re-uploaded; nothing in the app deletes a
// graphic from storage, so one event can't break another's picture.
export async function duplicateEvent(eventId, createdBy) {
  const [source, items, questions] = await Promise.all([
    getEvent(eventId),
  ...
  const closesStillAhead = new Date(source.closes_at) > new Date();

  const { data: copy, error } = await programming
    .from("events")
    .insert({
      title: source.title,
      body: source.body,
      image_path: source.image_path,
      event_date: source.event_date && source.event_date >= today ? source.event_date : null,
      closes_at: closesStillAhead ? source.closes_at : boiseInstantFrom(addDays(today, 14), "23:45"),
      location: source.location,
      target_type: source.target_type,
      target_group_program_id: source.target_group_program_id,
      response_type: source.response_type,
      link_url: source.link_url,
      ask_guest_count: source.ask_guest_count,
  ...
  try {
    if (items.length > 0) {
      const { error: itemsError } = await programming.from("event_items").insert(
        items.map((item) => ({
          event_id: copy.id,
          name: item.name,
    ...
    if (questions.length > 0) {
      const { error: questionsError } = await programming.from("event_questions").insert(
        questions.map((q) => ({
          event_id: copy.id,
    ...
    // Half a copy is worse than none: a draft with no items reads as a real
    // event somebody forgot to finish. The children cascade with it.
    await programming.from("events").delete().eq("id", copy.id);
    throw err;
  }
...
// Draft / scheduled / live / past, from the same three fields the
// member-facing RLS uses — so what a coach reads here can't disagree with
// what a member can actually see.
export function eventPhase(event, now = new Date()) {
  if (event.status !== "published") return "draft";
  if (new Date(event.closes_at) <= now) return "past";
  if (event.publish_at && new Date(event.publish_at) > now) return "scheduled";
  return "live";
}
...
export async function listLiveEventsForUser(userId) {
  // RLS already restricts this to published + started + not-yet-closed; the
  // audience filter is client-side, same as announcements.
  ...
  const { data, error } = await programming
    ...
    .eq("status", "published")
    .gt("closes_at", nowIso)
    // Mirrors the RLS gate rather than relying on it alone, same as the two
    // filters above — a scheduled event is invisible either way.
...
// Pure — what one set of line items costs. null when nothing in it has a
// price, so a screen can say nothing rather than "$0".
export function lineItemsTotal(lineItems, items) {
  const itemsById = Object.fromEntries(items.map((i) => [i.id, i]));
  let total = 0;
  let priced = false;
  for (const li of lineItems ?? []) {
    const price = itemsById[li.event_item_id ?? li.eventItemId]?.price;
    if (price === null || price === undefined) continue;
    priced = true;
    total += Number(price) * Number(li.qty);
  }
    ...
    for (const li of response.lineItems ?? []) {
      ...
      if (existing) {
        ...
        buckets.set(key, {
          ...
          qty: li.qty,
          people: 1,
          price: itemsById[li.event_item_id]?.price ?? null,
        });
      }
    }
  }
