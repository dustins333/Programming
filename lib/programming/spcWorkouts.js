...
export async function getSpcWorkout(workoutId) {
  const { data, error } = await programming
    .from("spc_workouts")
    .select("*, spc_blocks(id, spc_client_id, block_length_weeks, format, status)")
    .eq("id", workoutId)
    .single();
...
export async function listSpcWarmups(workoutId) {
  // default_sets/default_reps ride along so the print view can fall back to
  // the library default when a warm-up row was added before one existed.
  const { data, error } = await programming
...
// The same rows for a whole set of workouts at once — the mobile session
// preview shows one warm-up card per session tab and would otherwise fire a
// query per tab as a coach flicks between them.
export async function listSpcWarmupsForWorkouts(workoutIds) {
...
// Replace each target session's whole warm-up with `rows`. Deliberately
// source-agnostic: the builder's "Copy to..." passes another session's rows
// today, and warm-up templates are meant to pass their own rows through this
// same function later, so there is one definition of "set a session's
// warm-up". Only the movement fields are read off `rows`, so any row shape
// carrying exercise_id/label/sets/reps/notes/superset_group_id works.
//
// Superset ids are re-minted per target: the same uuid in two sessions would
// claim they are one superset, and nothing about a copy should link sessions.
export async function replaceSpcWarmups(toWorkoutIds, rows) {
  for (const workoutId of toWorkoutIds) {
    ...
    const { error: delError } = await programming.from("spc_workout_warmups").delete().eq("spc_workout_id", workoutId);
    if (delError) throw delError;
    if (rows.length === 0) continue;
    const { error } = await programming.from("spc_workout_warmups").insert(
      rows.map((w, i) => ({
        spc_workout_id: workoutId,
        exercise_id: w.exercise_id ?? null,
        position: i + 1,
        label: w.label ?? null,
        sets: w.sets ?? null,
        reps: w.reps ?? null,
        notes: w.notes ?? null,
        superset_group_id: remap(w.superset_group_id),
      }))
    );
    if (error) throw error;
  }
...
// Move a session to a different week of its own block, or clear the move
// (week = null) to put it back where it was written.
//
// The authored week_number is deliberately never touched: SPC completions are
// keyed on it (0007's unique index), so rewriting it would orphan every
// completion already recorded against this row. See 0101 for the full split
// between "which week is this in" and "which week is a completion filed
// under".
//
// The block-length clamp is enforced by a trigger as well as here — this
// check exists so a coach gets a sentence rather than a constraint error.
export async function setSpcWorkoutScheduledWeek(workoutId, week) {
  if (week == null) {
    const { error } = await programming.from("spc_workouts").update({ scheduled_week: null }).eq("id", workoutId);
    if (error) throw error;
    return;
  }
  const target = Math.floor(Number(week));
  if (!Number.isFinite(target) || target < 1) throw new Error("Pick a week in this block.");

  const { data: workout, error: workoutError } = await programming
    .from("spc_workouts")
    .select("id, week_number, spc_block_id")
    .eq("id", workoutId)
    .single();
  ...
  const { data: block, error: blockError } = await programming
    .from("spc_blocks")
    .select("block_length_weeks")
    .eq("id", workout.spc_block_id)
    .single();
  if (blockError) throw blockError;

  if (target > block.block_length_weeks) {
    throw new Error(
      `This block is ${block.block_length_weeks} weeks long, so there's no week ${target} to move it to. Extend the block first.`
    );
  }

  // Back to its authored week is a clear, not a move — so undoing leaves no
  // trace, same as the spec's rule that a move leaves none in the week it
  // came from.
  const value = target === workout.week_number ? null : target;
  const { error } = await programming.from("spc_workouts").update({ scheduled_week: value }).eq("id", workoutId);
  if (error) throw error;
}
