...
export async function listSpcWorkoutExercises(workoutId) {
  const { data, error } = await programming
    ...
    .select("*, exercises(id, name, muscle_group, movement_pattern, video_url, cues)")
    .eq("spc_workout_id", workoutId)
    .order("position");
  if (error) throw error;
  return data;
...
export async function addSpcWorkoutExercise({ workoutId, exerciseId, position, userId, notes }) {
  const recentLog = userId ? await getMostRecentLog(userId, exerciseId) : null;
  const sets = recentLog?.sets ?? 3;
  ...
  // literal text "null" instead of falling back to the 3x10 default.
  const reps = recentLog?.reps != null ? String(recentLog.reps) : "10";
...
// same as workouts.js's getSiblingPatterns, now that SPC has a real
// week_number per row like group does.
export async function getSpcSiblingPatterns(spcBlockId, weekNumber, excludeWorkoutId) {
  ...
  return rows.flatMap((r) => r.exercises?.movement_pattern ?? []);
}
