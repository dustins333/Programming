...
async function getGroupProgramDashboard() {
  ...
  return Promise.all(
    programs.map(async (program) => {
      ...
      // Week position, so a row can say "Week 4 of 4" rather than only how
      // many days are left. Both numbers are already computed above to
      // decide what's published — this just stops throwing them away.
      const lengthWeeks = currentBlock ? blockLengthWeeks(currentBlock, program) : null;

      return {
        programId: program.id,
        name: program.name,
        weekNumber: currentBlock ? currentWeekNumber(currentBlock.block_start_date, lengthWeeks, today) : null,
...
export async function getCoachDashboardStats() {
  ...
  const spcDue = spcRoster
    ...
    .map((c) => ({
      userId: c.userId,
      name: c.name,
      // coachId as well as the name: the dashboard scopes these rows to
      // whoever is looking (scopeSpcToCoach in launchpad.js), and a display
      // name is not an identity. Null means nobody is assigned to her.
  ...
  // The four SPC sub-tiles on both dashboards, straight off deriveSpcState —
  // the same four states the roster itself groups by, so a tile and the
  // filtered roster it opens can't report different numbers. Counted here
  // rather than in each dashboard because the phone and the desktop have to
  // agree, and that agreement is the whole point of the redesign.
  //
  // "No coach" is deliberately a different axis from the other three: it's
  // an ownership gap, not a programming state, so a client can be both Good
  // to go AND unowned and is counted in both.
  const spcStates = { dueNow: 0, dueSoon: 0, goodToGo: 0, paused: 0 };
  for (const c of spcRoster) if (c.state in spcStates) spcStates[c.state] += 1;
  const spcNoCoach = spcRoster.filter((c) => !c.coachId).length;
  ...
  for (const c of activeNutritionRoster) {
    ...
    // A week the coach closed out (0111) counts here alongside a reviewed
    // one, deliberately. These four buckets exist to answer "how much is
    // still outstanding", and a closed-out week is not; splitting it into a
    // fifth tile would grow this row for a state that is, by design, the
    // one nobody has to act on.
  ...
  return {
    ...
    spcIssues: spcDue,
    // Nobody has put anything in for the whole trailing week — the roster's
    // own rolling window (today-6..today), so it means the same thing here as
    // it does on the nutrition screens. Named, not just counted, because the
    // tile opens a list of exactly these people and a bare number would drop
    // a coach on a roster to go find them.
    nutritionNotSeen: activeNutritionRoster
      .filter((c) => !c.isOnboarding && c.daysLogged === 0)
      .map((c) => ({ userId: c.userId, name: c.name, coachName: c.coachName }))
