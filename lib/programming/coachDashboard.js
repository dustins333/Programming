...
export async function getCoachDashboardStats() {
  ...
  return {
    ...
    // Was `status === 'new_program_asap'`, a value the auto-draft stamped by
    // hand. The draft block it creates says the same thing without anyone
    // having to write it down (migration 0099).
    spcNeedsNewProgram: spcRoster.filter((c) => c.hasDraft).map((c) => ({ userId: c.userId, name: c.name })),
    // Everything on the SPC roster that a coach would want to do something
    // about, named and reasoned, ordered worst-first. Derived from the
    // roster already fetched above — no extra query. `reason` is short
    // enough to be a row's whole subtitle on a phone.
    spcIssues: spcRoster
      .map((c) => {
        if (c.hasDraft) return { ...c, severity: 0, reason: "Draft written, not sent" };
        if (c.overdue)
          return { ...c, severity: 1, reason: `Block ended ${Math.abs(c.daysUntilEnd)} day${Math.abs(c.daysUntilEnd) === 1 ? "" : "s"} ago` };
        if (c.dueSoon && c.daysUntilEnd !== null)
          return { ...c, severity: 2, reason: `Block ends in ${c.daysUntilEnd} day${c.daysUntilEnd === 1 ? "" : "s"}` };
        // everScheduled excludes the ~51 clients who hold an spc_clients row
        // from the enrolment toggle and have never had a block written. They
        // are un-programmed, not behind, and listing them here buried the
        // handful who genuinely need something (same call migration 0084 made
        // for the hub picker). They still show on the roster as "Not started".
        if (!c.currentBlock && c.status !== "paused" && c.everScheduled)
          return { ...c, severity: 3, reason: "No active block" };
        return null;
      })
      .filter(Boolean)
      .sort((a, b) => a.severity - b.severity)
      .map((c) => ({ userId: c.userId, name: c.name, reason: c.reason, severity: c.severity, coachName: c.coachName })),
