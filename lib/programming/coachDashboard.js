...
export async function getCoachDashboardStats() {
  ...
  return {
    ...
    spcIssues: spcRoster
      .map((c) => {
        if (c.status === "new_program_asap") return { ...c, severity: 0, reason: "Needs a new program" };
        if (c.overdue)
        ...
        if (!c.currentBlock && c.status !== "paused") return { ...c, severity: 3, reason: "No active block" };
        return null;
      })
      .filter(Boolean)
