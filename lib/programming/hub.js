...
// Which week a completion for this workout is filed under. Mirrors
// spcCompletionWeek() in sessionCompletions.js — kept as a local rather than
// imported because that one takes a round trip to fetch the row this function
// already has in hand, and the board polls every three seconds.
...
// `dateOverride` is what makes a PAST board reviewable: the logs half of this
// query is date-scoped, and a finished session's sets sit on the day it ran,
// not today. Everything else (structure, completions, notes) is current state
// by design — a lift ticked off yesterday is still ticked off.
export async function fetchHubBoard(slots, dateOverride = null) {
  ...
  if (spcIds.length > 0) {
    ...
    add("spcWorkouts", programming.from("spc_workouts").select("id, title, session_number, week_number, spc_block_id").in("id", spcIds));
  ...
  if (spcItemIds.length > 0) {
    compQ.push(programming.from("exercise_completions").select("user_id, spc_workout_exercise_id, week_number").in("spc_workout_exercise_id", spcItemIds));
  }
  if (groupItemIds.length > 0) {
    compQ.push(programming.from("exercise_completions").select("user_id, group_workout_exercise_id").in("group_workout_exercise_id", groupItemIds));
  }
  ...
  for (const slot of slots) {
    ...
    const completedItemIds = new Set(
      exerciseCompRows
        .filter((r) => r.user_id === slot.user_id && itemIdSet.has(r[itemCol]))
        .map((r) => r[itemCol])
    );

    const finalized = sessionComp.some((r) => r.user_id === slot.user_id && r[workoutCol] === ref.workoutId);
    ...
    for (const note of notes) {
      ...
      if (note[workoutCol] === ref.workoutId && !noteForWeekByExerciseId.has(note.exercise_id)) {
...
export async function getLiftBlockHistory({ userId, exerciseId, blockId, kind = "spc", excludeWorkoutId = null }) {
  ...
  const { data: workouts, error: workoutsError } = group
    ...
    : await programming.from("spc_workouts").select("id, week_number").eq("spc_block_id", blockId);
  ...
  const [logsRes, notesRes] = await Promise.all([
    ...
    programming
      ...
      .select(`${workoutCol}, body, author_name, created_at`)
      .eq("user_id", userId)
      .eq("exercise_id", exerciseId)
      .in(workoutCol, ids)
      .order("created_at", { ascending: false }),
  ...
  for (const note of notesRes.data ?? []) {
    if (!noteByWorkout.has(note[workoutCol])) noteByWorkout.set(note[workoutCol], note);
  }

  const byWorkout = new Map();
  for (const row of logsRes.data ?? []) {
    if (row.reps == null && row.weight == null) continue; // cleared rows aren't history
    if (!byWorkout.has(row[workoutCol])) byWorkout.set(row[workoutCol], []);
    byWorkout.get(row[workoutCol]).push(row);
  }
