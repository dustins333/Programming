...
export function slotSession(slot) {
  ...
  const oneOffWorkoutId = slot.one_off_workout_id ?? null;
  // A one-off (0128) is the third kind. It has no block and no week: it is one
  // session copied onto one client, open until she finishes it.
  const kind = oneOffWorkoutId ? "one_off" : groupWorkoutId ? "group" : "spc";
  return {
    kind,
    workoutId: oneOffWorkoutId ?? groupWorkoutId ?? slot.spc_workout_id,
    spcWorkoutId: kind === "spc" ? slot.spc_workout_id : null,
    groupWorkoutId,
    oneOffWorkoutId,
  };
}
...
export function sessionRefFor(entry) {
  if (entry.kind === "one_off") return { oneOffWorkoutId: entry.oneOffWorkoutId };
  return entry.kind === "group"
    ? { groupWorkoutId: entry.groupWorkoutId }
    : { spcWorkoutId: entry.spcWorkoutId, weekNumber: entry.weekNumber };
}

// A make-up: she has already logged this session this week and is doing it
// again, so it needs a SECOND completion rather than re-stamping the first.
//
// Goes through the RPC rather than inserting here, because this has to work
// from the wall display too — whose only write policy on session_completions
// is "is_gym_display() AND the client is on the open board". That is exactly
// why it runs AFTER the board write and not from the picker, where every
// attempt from the TV was refused with a 42501 (see migration 0117).
//
// Does nothing, and returns null, when she has no completion for that week
// after all — a staged answer can be a day stale, and inventing a completion
// for a session she has not done would open the board washed green and count
// a session that never happened.
async function openMakeup({ userId, spcWorkoutId, weekNumber }) {
  const { data, error } = await programming.rpc("hub_open_makeup", {
    p_user_id: userId,
...
// Every slot the coach answered "start a new one" for. Group slots have no
// instance concept (0040 requires a null week on a group completion), so they
// are skipped rather than silently mis-filed.
//
// Only startHubSession still needs this — it writes the slots directly rather
// than through an RPC, so there is nowhere else to put the call. It creates a
// new session id, and clientsSignature now covers instance, so the board picks
// the stamp up either way. The two RPC paths resolve it server-side instead.
async function openMakeupsFor(slots) {
  for (const slot of slots) {
    if (!slot?.newInstance) continue;
    if (slot.groupWorkoutId || slot.oneOffWorkoutId || !slot.spcWorkoutId) continue;
    await openMakeup({ userId: slot.userId, spcWorkoutId: slot.spcWorkoutId, weekNumber: slot.weekNumber });
  }
}
...
export async function startHubSession({ coachId, coachName = null, slots }) {
  ...
  const rows = slots.map((slot, i) => ({
    ...
    spc_workout_id: slot.groupWorkoutId || slot.oneOffWorkoutId ? null : slot.spcWorkoutId,
    group_workout_id: slot.groupWorkoutId ?? null,
    one_off_workout_id: slot.oneOffWorkoutId ?? null,
    // NOT NULL on the table; a one-off has no week, so it carries 1 (0128).
    week_number: slot.weekNumber ?? 1,
    position: i + 1,
  }));
...
export async function fetchHubWarmups(slots) {
  const spcIds = slots.filter((s) => slotSession(s).kind === "spc").map((s) => s.spc_workout_id);
  const groupIds = slots.filter((s) => s.group_workout_id).map((s) => s.group_workout_id);
  const oneOffIds = slots.filter((s) => s.one_off_workout_id).map((s) => s.one_off_workout_id);
  ...
  if (oneOffIds.length > 0) {
    queries.push(
      programming.from("one_off_warmups").select(WARMUP_SELECT).in("one_off_workout_id", oneOffIds).order("position")
    );
  }
  ...
  for (const res of results) {
    ...
    for (const row of res.data ?? []) {
      const workoutId = row.one_off_workout_id ?? row.group_workout_id ?? row.spc_workout_id;
...
//   instance,              // which completion of this session the board runs (0118)
...
//   finalized,             // bool (a completion exists for this week AND instance)
...
export async function fetchHubBoard(slots, dateOverride = null) {
  ...
  if (spcIds.length > 0) {
    ...
    add("spcWorkouts", programming.from("spc_workouts").select("id, title, session_number, week_number, spc_block_id, spc_blocks(format, block_start_date, block_end_date, block_length_weeks)").in("id", spcIds));
    add("spcLogs", programming.from("logs").select("*").in("spc_workout_id", spcIds).eq("date_performed", today).order("set_number"));
    add("spcSessionComp", programming.from("session_completions").select("user_id, spc_workout_id, week_number, instance, completed_at").in("spc_workout_id", spcIds));
  }
  if (groupIds.length > 0) {
  ...
  // A one-off (0128) reads its own three tables. No block, no week, no
  // instance: one row per session, finished once.
  if (oneOffIds.length > 0) {
    add("oneOffStructure", programming.from("one_off_exercises").select(EXERCISE_SELECT).in("one_off_workout_id", oneOffIds).order("position"));
    add("oneOffWorkouts", programming.from("one_off_workouts").select("id, title").in("id", oneOffIds));
    add("oneOffLogs", programming.from("logs").select("*").in("one_off_workout_id", oneOffIds).eq("date_performed", today).order("set_number"));
    add("oneOffSessionComp", programming.from("session_completions").select("user_id, one_off_workout_id, completed_at").in("one_off_workout_id", oneOffIds));
  }
  ...
  const structure = [...(got.spcStructure ?? []), ...(got.groupStructure ?? []), ...(got.oneOffStructure ?? [])];
  const logs = [...(got.spcLogs ?? []), ...(got.groupLogs ?? []), ...(got.oneOffLogs ?? [])];
  const sessionComp = [...(got.spcSessionComp ?? []), ...(got.groupSessionComp ?? []), ...(got.oneOffSessionComp ?? [])];
  ...
  const oneOffItemIds = (got.oneOffStructure ?? []).map((r) => r.id);
  const compQ = [];
  if (oneOffItemIds.length > 0) {
    compQ.push(programming.from("exercise_completions").select("user_id, one_off_exercise_id, completed_at").in("one_off_exercise_id", oneOffItemIds));
  }
  if (spcItemIds.length > 0) {
    compQ.push(programming.from("exercise_completions").select("user_id, spc_workout_exercise_id, week_number, instance, completed_at").in("spc_workout_exercise_id", spcItemIds));
  }
  ...
  for (const w of got.oneOffWorkouts ?? []) workoutMeta.set(w.id, { ...w, blockId: null });
  ...
  for (const slot of slots) {
    ...
    const workoutCol =
      ref.kind === "one_off" ? "one_off_workout_id" : ref.kind === "group" ? "group_workout_id" : "spc_workout_id";
    const itemCol =
      ref.kind === "one_off" ? "one_off_exercise_id" : ref.kind === "group" ? "group_workout_exercise_id" : "spc_workout_exercise_id";
    ...
    const completionWeek = ref.kind === "spc" ? completionWeekFor(meta, today) : null;
    const inWeek = (r) =>
      ref.kind !== "spc" || completionWeek == null || (r.week_number ?? null) === completionWeek;

    // Which of her completions of that session this board is running. 1 for
    // every ordinary slot; higher only when the coach answered "start a new
    // one" (0118). Without it a make-up inherits the earlier session's
    // finalized state and its ticks, and opens looking already done. Group
    // slots have no instance concept, so they skip the check.
    const slotInstance = slot.instance ?? 1;
    const inInstance = (r) => ref.kind !== "spc" || (r.instance ?? 1) === slotInstance;

    const itemIdSet = new Set(items.map((i) => i.id));
    const completedItemIds = new Set(
      exerciseCompRows
        .filter((r) => r.user_id === slot.user_id && itemIdSet.has(r[itemCol]) && inWeek(r) && inInstance(r) && onBoardDay(r))
        .map((r) => r[itemCol])
    );

    const finalized = sessionComp.some(
      (r) => r.user_id === slot.user_id && r[workoutCol] === ref.workoutId && inWeek(r) && inInstance(r) && onBoardDay(r)
    );
    ...
    // How many times the block asks for it in total, so the pill reads
    // "3 of 8". Null means an ONGOING program (0103: sessions format, no end
    // date) — there is no total, and the card says so with an infinity mark
    // rather than inventing a number off block_length_weeks, which is left
    // populated but unread while the end is null.
    let sessionRunTotal = null;
    if (ref.kind === "spc" && meta?.spc_blocks?.format === "sessions" && completionWeek != null) {
      ...
      // block_length_weeks is NOT NULL, so the only way this comes back
      // null is a genuinely ongoing program.
      sessionRunTotal = meta.spc_blocks.block_end_date ? meta.spc_blocks.block_length_weeks : null;
    }
    ...
    board.set(slot.user_id, {
      ...
      oneOffWorkoutId: ref.oneOffWorkoutId,
      blockId: meta?.blockId ?? null,
      // A one-off has no week; the slot's stored 1 is a placeholder (0128).
      weekNumber: ref.kind === "one_off" ? null : slot.week_number,
      ...
      completionWeek,
      // Which of her completions of this session the board is running (0118).
      // Writers must pass it, or a make-up's finalize would re-stamp the
      // earlier session instead of recording a second one.
...
export async function getLiftBlockHistory({
  ...
  const { data: workouts, error: workoutsError } = group
    ...
    : await programming
        .from("spc_workouts")
        .select("id, week_number, spc_blocks(format, block_start_date, block_end_date, block_length_weeks)")
  ...
  const [logsRes, notesRes] = await Promise.all([
    programming
      .from("logs")
      .select(`${workoutCol}, set_number, reps, weight, date_performed, notes, set_type`)
      .eq("user_id", userId)
...
export async function startHubSessionWithPin({ pin, slots }) {
  const { data, error } = await programming.rpc("hub_start_session", {
    ...
    p_clients: slots.map((s) => ({
      userId: s.userId,
      spcWorkoutId: s.groupWorkoutId || s.oneOffWorkoutId ? null : s.spcWorkoutId,
      groupWorkoutId: s.groupWorkoutId ?? null,
      oneOffWorkoutId: s.oneOffWorkoutId ?? null,
      weekNumber: s.weekNumber,
      // Resolved inside the RPC, before the slot is visible to anyone (0119).
      // Stamping it as a follow-up write left a window the board's own poll
      // fell into, and once it had cached the slot at instance 1 nothing ever
      // changed the roster again to make it look twice.
      newInstance: Boolean(s.newInstance),
    })),
  });
  if (error) throw error;
  return data;
...
export async function addHubClient({
  userId,
  spcWorkoutId = null,
  groupWorkoutId = null,
  oneOffWorkoutId = null,
  weekNumber,
  newInstance = false,
}) {
  const { error } = await programming.rpc("hub_add_client", {
    p_user_id: userId,
    p_spc_workout_id: groupWorkoutId || oneOffWorkoutId ? null : spcWorkoutId,
    p_group_workout_id: groupWorkoutId,
    p_one_off_workout_id: oneOffWorkoutId,
    p_week_number: weekNumber,
    // Same as above, and this is the case that made it certain rather than a
    // race: adding a client changes the roster, so the poll refreshes on the
    // spot — at instance 1 — and the follow-up stamp then changed nothing it
    // would ever notice.
    p_new_instance: Boolean(newInstance),
  });
  if (error) throw error;
}
