...
export async function fetchHubBoard(slots) {
  ...
  const [structureRes, workoutsRes, logsRes, sessionCompRes, notesRes] = await Promise.all([
    ...
    programming
      .from("spc_workouts")
      .select("id, title, session_number, week_number, spc_block_id, spc_blocks(format, block_start_date)")
      .in("id", workoutIds),
    programming
      .from("logs")
      .select("*")
  ...
  for (const slot of slots) {
    ...
    // Scoped to THIS week's completion, not "a completion exists". For a
    // weekly block the workout row is already unique to its week, so the two
    // are the same question — but a sessions-format run (0102) has one row
    // per session that recurs every week, and without the week filter a
    // session finalized in week 1 would read as finalized on the board
    // forever afterwards.
    //
    // The week is resolved the same way spcCompletionWeek does it: the
    // authored week for a weekly block, the uncapped calendar week for a
    // sessions run.
    const finalized = (sessionCompRes.data ?? []).some(
      (r) =>
        r.user_id === slot.user_id &&
        r.spc_workout_id === slot.spc_workout_id &&
        r.week_number === completionWeekFor(workoutMeta.get(slot.spc_workout_id), today)
    );
