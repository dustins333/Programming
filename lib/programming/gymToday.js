...
// Total load moved across the gym today. One fat-fingered weight used to be
// able to move this figure by a fifth on its own, so the sum goes through
// the shared setVolume guard — see lib/programming/volume.js for the
// threshold and the real numbers behind it.
async function volumeToday(today) {
  const { data, error } = await programming
    .from("logs")
    .select("reps, weight, exercises(tracks_weight, rep_unit)")
    .eq("date_performed", today)
    .limit(5000);
  if (error) throw error;
  return sumVolume(data);
}
