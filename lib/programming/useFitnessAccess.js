...
// membership, active SPC, or an open one-off. Backs hiding the My Fitness
...
export function useHasFitness(userId) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [assignments, spcClient, oneOffs] = await Promise.all([
          listMyAssignments(userId),
          getSpcClient(userId),
          listActiveOneOffWorkoutsForUser(userId),
        ]);
        if (!cancelled) {
          setHasFitness(assignments.length > 0 || isSpcActive(spcClient) || oneOffs.length > 0);
        }
      } catch {
