...
import { getConditioningClient, isConditioningActive } from "./conditioning";
...
export function useHasFitness(userId) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [assignments, spcClient, oneOffs, alternate, conditioning] = await Promise.all([
          listMyAssignments(userId),
          getSpcClient(userId),
          listActiveOneOffWorkoutsForUser(userId),
          getLiveAlternateProgramForUser(userId),
          // Own catch: a conditioning-only failure must not flip the whole
          // answer to the "error" default when the rest resolved.
          getConditioningClient(userId).catch(() => null),
        ]);
        if (!cancelled) {
          setHasFitness(
            assignments.length > 0 ||
              ...
              oneOffs.length > 0 ||
              !!alternate ||
              isConditioningActive(conditioning)
          );
        }
      } catch {
