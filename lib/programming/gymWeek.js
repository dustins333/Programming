...
import { describeConditioningLog } from "./conditioning";
...
async function trainingRoster() {
  const [{ data: assignments, error: aError }, { data: spc, error: sError }, conditioning] = await Promise.all([
    programming.from("client_program_assignments").select("user_id"),
    programming.from("spc_clients").select("user_id, status"),
    // Conditioning counts as training (0130). Isolated: a failure here only
    // drops conditioning-only clients from the roster, never the whole band.
    programming
  ...
  for (const c of conditioning) if (c.status === "active") ids.add(c.user_id);
  return [...ids];
}

// This week's conditioning sessions, shaped like a finalized lifting session
// so the counts and the sheet treat them the same. Logging one IS finishing
// it; there's no separate finalize step.
async function conditioningSessionsSince(weekStart) {
  const { data, error } = await programming
    .from("conditioning_logs")
    .select("id, user_id, performed_on, duration_minutes, avg_heart_rate, feel, created_at")
  ...
  return (data ?? []).map((log) => ({
    ...
    label: "Conditioning",
    sessionTitle: describeConditioningLog(log, { separator: " · " }),
    session: null,
    conditioning: true,
    finalized: true,
  }));
...
export async function getGymWeek(today = todayInBoise()) {
  ...
  const [{ data: comps, error: compError }, rosterIds, conditioningSessions] = await Promise.all([
    programming
      ...
      .limit(2000),
    trainingRoster(),
    conditioningSessionsSince(weekStart).catch(() => []),
  ]);
  if (compError) throw compError;

  const liftingSessions = (comps ?? [])
    .map((c) => ({
      id: c.id,
      userId: c.user_id,
  ...
  const rawSessions = [...liftingSessions, ...conditioningSessions].sort((a, b) =>
