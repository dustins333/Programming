...
import { programEndDate } from "./alternatePrograms";
import { todayInBoise, dayOfWeekInBoise, addDays } from "../boiseDate";
...
async function addSpcFlags(flagsByUser) {
  ...
  const { data: workouts, error: workoutsError } = await programming
    .from("spc_workouts")
    .select("id, spc_block_id, week_number, scheduled_week, session_number, status")
    .in("spc_block_id", blockIds);
  if (workoutsError) throw workoutsError;

  // Effective week, not authored week (0101). A session the coach moved out of
  // the checked week isn't missed — it was rescheduled — and one moved INTO it
  // is genuinely due that week and should be checked. The completion lookup
  // below still keys on the AUTHORED week, because that is what
  // session_completions stores.
  const relevantWorkouts = workouts.filter((w) => {
    const entry = checkByBlockId[w.spc_block_id];
    return (
      entry &&
      (w.scheduled_week ?? w.week_number) === entry.checkWeek &&
      w.status === "published" &&
      w.session_number <= entry.cap
    );
  });
  ...
  for (const workout of relevantWorkouts) {
    ...
    if (completions.has(`${entry.userId}:${workout.id}:${workout.week_number}`)) continue;
    ...
    flags.push({
      ...
      period: "last week",
      // SPC's check is retrospective (the week that just finished), so the
      // shield has to be tested against that week, not today. A client who
      // got back on Monday shouldn't be flagged for the week she was away.
      checkDate: addDays(today, -7),
    });
...
export async function getMissedSessionFlagsByUser() {
  ...
  await Promise.all(
    programs.map(async (program) => {
      ...
      const weekNum = currentWeekNumber(currentBlock.block_start_date, blockLengthWeeks(currentBlock, program), today);
  ...
  for (const assignment of assignments) {
    ...
    for (const workout of entry.workouts) {
      ...
      flags.push({
        ...
        sessionNumber: workout.session_number,
        period: "this week",
        // Which day this flag is actually about, so a shielding alternate
        // program can be matched against the right week — see
        // dropFlagsShieldedByAlternatePrograms below.
        checkDate: today,
      });
  ...
  await dropFlagsShieldedByAlternatePrograms(flagsByUser);
  return flagsByUser;
}

// A client on alternate programming (0110) with "don't mark their normal
// sessions missed while this runs" ticked is travelling, not slacking. Her
// Flagship/SPC sessions still SHOW the whole time — she keeps her tile and
// her block page — they just cannot be reported as missed here, which is
// what feeds the Clients roster's Needs column, the flag pills and the
// client detail's Current/Behind.
//
// Matched per flag rather than per client, because the two scans ask about
// different weeks: the group scan is about this week, the SPC scan about
// the one that just ended. A run covering last week but ending Sunday must
// still clear last week's SPC flags, and must not clear this week's group
// ones.
async function dropFlagsShieldedByAlternatePrograms(flagsByUser) {
  if (flagsByUser.size === 0) return;

  const { data: programs, error } = await programming
    .from("alternate_programs")
    .select("user_id, start_date, weeks, ended_at, pause_missed_flags")
    .eq("pause_missed_flags", true)
    .in("user_id", [...flagsByUser.keys()]);
  if (error) throw error;
  if (!programs.length) return;

  const byUser = new Map();
  for (const program of programs) {
    const ranges = byUser.get(program.user_id) ?? [];
    ranges.push({ start: program.start_date, end: programEndDate(program) });
    byUser.set(program.user_id, ranges);
  }

  for (const [userId, flags] of flagsByUser) {
    const ranges = byUser.get(userId);
    if (!ranges) continue;
    const kept = flags.filter(
      (flag) => !ranges.some((r) => flag.checkDate >= r.start && flag.checkDate <= r.end)
    );
    if (kept.length === 0) flagsByUser.delete(userId);
    else flagsByUser.set(userId, kept);
  }
}
