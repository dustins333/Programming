...
// Where a program lands on the calendar, and what that does to whatever else
// is scheduled. Shared by publish and by rescheduling an already-published
// one, because those two answer the identical question and a second copy is
// exactly how the "Publish" and "Change dates" paths would drift apart on
// which programs get shortened.
async function resolveProgramDates(block, { requestedStart, lengthWeeks = null, ongoing = false }) {
  if (!requestedStart) throw new Error("Pick the Monday this program starts on.");
  ...
  // A sessions-format program that starts before the current one ends
  // SHORTENS the current one to end the day before — the modal says so in
  // words and that sentence is the confirmation (design handoff v1, open
  ...
  if (block.format === "sessions") {
    await endProgramsBefore(block.spc_client_id, startDate, endDate, block.id);
  }
  ...
  await assertNoOverlap(block.spc_client_id, startDate, endDate, block.id);
  return { startDate, endDate, weeks };
}

// Publish whatever is ready and isn't live yet: every session holding at least
// one lift. An empty session stays hidden, because publishing it would put a
// blank card on her My Week.
//
// This is the one definition of "what goes live", used by publish, by the
// current program's Update button, and by a queued program's — a session added
// after publishing would otherwise sit as a draft forever, invisible to her
// with nothing on screen saying so.
export async function publishReadySessions(blockId) {
  const workouts = await listSpcWorkoutsForBlock(blockId);
  const workoutIds = workouts.map((w) => w.id);
  if (workoutIds.length === 0) return { published: 0, empty: 0 };
  const { data: lifts, error } = await programming
    .from("spc_workout_exercises")
    .select("spc_workout_id")
    .in("spc_workout_id", workoutIds);
  if (error) throw error;
  const withLifts = new Set(lifts.map((row) => row.spc_workout_id));
  ...
  return { published: toPublish.length, empty: workouts.length - withLifts.size, ready: withLifts.size };
}

// A guard both of the queued-program operations need: they are only safe
// BEFORE the program starts. Once it is live, changing its dates is
// moveSpcBlock/setSpcProgramEnd, and unpublishing it would take a program out
// from under someone mid-week.
async function assertNotStarted(block, verb) {
  if (block.status !== "active") throw new Error("This program hasn't been published yet.");
  if (!block.block_start_date || block.block_start_date <= todayInBoise()) {
    throw new Error(`This program has already started, so it can't be ${verb}.`);
  }
}

// Change a published-but-not-yet-started program's dates. She published it for
// the wrong Monday, or the length was wrong, and before this there was simply
// no way back: the pane showed "Published, goes live Monday" and no controls
// at all.
//
// Safe by construction — the member cannot see a queued program (0102 gates
// her reads on block_start_date <= today), so nothing she has already seen
// changes. Moving it EARLIER, including onto this week so it goes live now,
// runs the same shortening rules a fresh publish would.
export async function rescheduleSpcProgram(blockId, { startDate: requestedStart, lengthWeeks = null, ongoing = false }) {
  const block = await getSpcBlock(blockId);
  await assertNotStarted(block, "rescheduled");
  const { startDate, endDate, weeks } = await resolveProgramDates(block, { requestedStart, lengthWeeks, ongoing });

  await endRollingBlocksBefore(block.spc_client_id, startDate);
  const { error } = await programming
    .from("spc_blocks")
    .update({ block_start_date: startDate, block_end_date: endDate, block_length_weeks: weeks })
    .eq("id", blockId);
  if (error) throw error;
  return { startDate, endDate, ongoing: endDate == null };
}

// Take a queued program back out of the schedule. It returns to the build
// space as a draft with everything in it intact, which is the same shape
// endProgramsBefore uses when a new program replaces one that hasn't started
// — so "cancel" and "replaced" leave a program in exactly the same state, and
// there is one thing to understand rather than two.
//
// Deliberately NOT a delete: she built those sessions. Deleting is the
// separate, explicit action already on a draft.
export async function unpublishSpcProgram(blockId) {
  const block = await getSpcBlock(blockId);
  await assertNotStarted(block, "cancelled");
  // Belt to the not-started brace. A queued program is invisible to her, so
  // this should be impossible; if it ever isn't, refusing beats quietly
  // stranding a completion against a program that no longer has dates.
  const workouts = await listSpcWorkoutsForBlock(blockId);
  if (workouts.length > 0) {
    const { count, error } = await programming
      ...
      .in("spc_workout_id", workouts.map((w) => w.id));
    if (error) throw error;
    if ((count ?? 0) > 0) throw new Error("There are logged sessions in this program, so it can't be cancelled.");
  }
  const { error } = await programming
    .from("spc_blocks")
    .update({ status: "draft", block_start_date: null, block_end_date: null })
...
export async function publishSpcBlock(blockId, { startDate: requestedStart, lengthWeeks = null, ongoing = false }) {
  ...
  const { ready, empty } = await publishReadySessions(blockId);
