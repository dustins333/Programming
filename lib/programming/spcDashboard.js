...
export async function checkAndAutoDraft() {
  ...
  for (const block of blocks) {
    // Drafts carry no dates, so they can't be "the latest block" — but their
    // existence is what stops this from drafting a second one behind them.
    if (block.status === "draft") {
      hasDraft.add(block.spc_client_id);
      continue;
    }
    if (!latestBlockByClient.has(block.spc_client_id)) latestBlockByClient.set(block.spc_client_id, block);
  }
  ...
  for (const client of clients) {
    ...
    // A coach has already written the next block and just hasn't sent it.
    // Auto-drafting another one on top would be exactly the duplicate churn
    // this whole feature exists to remove.
    if (hasDraft.has(client.user_id)) continue;
