...
import { clearAllScreenCaches } from "../screenCache";
import { clearRosterViews } from "../rosterViewState";
...
export function AuthProvider({ children }) {
  ...
  const loadProfile = useCallback(async (currentSession) => {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ({ data, error } = await core
        .from("users")
        .select("id, name, email, role, phone, created_at, is_gym_display, can_view_spc, can_view_nutrition, can_view_exercise_library, can_log_ops_hours, notify_daily_log_reminder, notify_checkin_available, notify_coach_messages")
  ...
  // Cached screen data is dropped before the sign-out itself, not after: once
  // the session is gone the auth listener re-renders the tree immediately, and
  // a member's cached My Week must never be sitting in storage for whoever
  // signs in next on a shared device. clearAllScreenCaches can't reject.
  const signOut = useCallback(async () => {
    clearRosterViews();
    await clearAllScreenCaches();
