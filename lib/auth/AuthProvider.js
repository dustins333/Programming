...
export function AuthProvider({ children }) {
  ...
  const loadProfile = useCallback(async (currentSession) => {
    ...
    const { data, error } = await core
      .from("users")
      .select("id, name, email, role, phone, can_view_spc, can_view_nutrition, can_view_exercise_library, notify_daily_log_reminder, notify_checkin_available, notify_coach_messages")
