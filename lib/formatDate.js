...
// The same date with no leading zeros: "9/4", not "09/04". For a small
// annotation hanging off something else — a note's byline, a corner badge —
// where the zeros are two characters of nothing. formatDateMD keeps its
// padding for the calendar grids, where the columns are fixed and the dates
// have to line up under each other.
export function formatDateMDShort(isoDate) {
  if (!isoDate) return "";
  const [, month, day] = isoDate.split("-");
  if (!month || !day) return isoDate;
