...
export async function saveRateSnapshotForPeriod(periodStart, { coreRates, otherRates, spcTiers }) {
  // Upsert, not insert: pay_period_start is unique, and this now runs
  // *before* the irreversible close so that a failure is recoverable — which
  // means a retry must not 23505 on the row the first attempt already wrote.
  const { error } = await payroll
    .from("closed_period_rate_snapshots")
    .upsert(
      { pay_period_start: periodStart, core_rates: coreRates, other_rates: otherRates, spc_tiers: spcTiers },
      { onConflict: "pay_period_start" }
    );
  if (error) throw error;
}
