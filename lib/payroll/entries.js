...
// period start. One query instead of one per period — the closed-periods
...
export async function listEntriesForPeriods(periodStarts) {
  ...
  const { data, error } = await payroll.from("pay_entries").select("*").in("pay_period_start", periodStarts);
  if (error) throw error;
