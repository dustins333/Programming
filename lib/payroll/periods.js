...
// What a period picker on the Log / Extra pay screens actually offers:
// the current period, plus the one before it while it's still open to this
// coach. Narrower than listWritablePeriods on purpose — that one mirrors
// what the DATABASE will accept, which is every open period, and a period
// that was simply never closed stays writable forever. In practice that
// meant a stale period from two cycles back sitting in the pill row next to
// the two live ones with nothing to say why (2026-07-23 was open next to a
// closed 2026-08-06 for weeks), reading as a period you were being invited
// to key money into.
//
// A coach who genuinely needs to reach further back gets there the same way
// they do after finalizing: an admin sends the period back, which reopens
// it. There is deliberately no self-serve path to an older period.
export function listSelectablePeriods(periods, currentPeriodStart, finalizations = null) {
  const writable = listWritablePeriods(periods, finalizations);
  const previousStart = addDays(currentPeriodStart, -PERIOD_LENGTH_DAYS);
  return writable
    .filter((p) => p.start_date === currentPeriodStart || p.start_date === previousStart)
