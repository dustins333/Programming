...
export const colors = {
  ...
  canvas: "#faf8f6",
  // The coach app's own, darker ground. White cards on plain canvas are a
  // 1.06:1 step, so the gutter between two cards reads as the same colour as
  // the cards and they run together; this is ~3x that separation, which is
  // what makes a card look like a card. Both dashboards shipped on it first
  // and the SPC screens now sit on it too — one token, because two surfaces
  // disagreeing about their own background is exactly the drift worth
  // preventing. colors.canvas is untouched: the whole member app sits on it.
  coachCanvas: "#f1ece6",
  // The card edge that goes WITH coachCanvas. The member app's #ece7e1 border
  // is only 1.05:1 against that darker ground — effectively invisible — so a
  // card would lose its edge the moment the ground moved. This holds 1.18:1,
  // and is what the dashboards have always used.
  coachCardBorder: "#e1dad1",
  ...
  muted: "#6f6862",
  // The dark surface. Every band, hero and running-board state in the app is
  // this one brown-black — it was repeated as a literal in a dozen files
  // before the coach dashboard redesign named it.
  ink: "#33251f",
  // Primary reading text on a light surface. Darker than `ink` on purpose:
  // ink is a FILL, this is TYPE, and the two are not the same colour.
  text: "#2a211c",
