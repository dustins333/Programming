...
export class LargeSecureStore {
  // Two concurrent first-ever writes (nothing in SecureStore yet — e.g. a
  // fresh login racing the SDK's own post-auth session save) could each see
  // "no key", generate a different one, and both write — whichever write
  // loses leaves the other call's already-encrypted ciphertext permanently
  // undecryptable under the key that actually ends up persisted. Memoizing
  // the in-flight creation promise per key makes every concurrent caller
  // await the same key instead of racing to create their own.
  _keyCreation = {};
  ...
  async _getOrCreateKey(secureStoreKey) {
    ...
    if (!this._keyCreation[secureStoreKey]) {
      this._keyCreation[secureStoreKey] = (async () => {
        ...
        delete this._keyCreation[secureStoreKey];
      });
    }
    return this._keyCreation[secureStoreKey];
  }
