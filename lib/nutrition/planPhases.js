...
export async function createPhase(userId, { title, details }, createdBy) {
  ...
  // Returns the inserted row so the caller can drop it straight into its own
  // list instead of refetching to find out what it just created — that
  // refetch was a full page reload, and the new card visibly disappeared for
  // the length of it.
...
// planned / now / done — a coach-set status, not something derived from
// `position`. Order says which phase comes next; status says which one she
// is actually in right now, and those genuinely diverge (two themes run side
// by side, or a phase gets parked mid-way). Also means a finished phase can
// stop reading as upcoming without being deleted, which used to be the only
// way to retire one — and threw away the history of what the client had
// already worked through. See migration 0059.
export const PHASE_STATUSES = ["planned", "now", "done"];

export async function setPhaseStatus(phaseId, status) {
  if (!PHASE_STATUSES.includes(status)) throw new Error(`Unknown phase status: ${status}`);
  const { error } = await programming.from("nutrition_plan_phases").update({ status }).eq("id", phaseId);
  if (error) throw error;
}
...
export async function addPhaseItem(phaseId, text) {
  ...
  // Returns the inserted row, same reasoning as createPhase above.
  const { data, error } = await programming
    .from("nutrition_plan_phase_items")
