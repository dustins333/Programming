...
export async function createMilestone(userId, { title, details }, createdBy) {
...
export async function updateMilestone(milestoneId, { title, details }) {
  const trimmedTitle = String(title || "").trim();
  if (!trimmedTitle) throw new Error("Milestone title is required");
  const { error } = await programming
    .from("nutrition_milestones")
    .update({ title: trimmedTitle, details: String(details || "").trim() || null })
    .eq("id", milestoneId);
  if (error) throw error;
}
