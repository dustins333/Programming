...
// "Week 11" — how far into the program this client is, counted from their
// own start_date. Weeks are 1-based (their first week is week 1, not week 0).
//
// A date BEFORE the start date returns null, not 1. This used to clamp to 1,
// which read fine for the one caller passing `today` but was wrong for the
// Weeks tab: every week that ended before the client's start date came back
// as "Week 1", so a client whose start_date sat near today (e.g. one created
// by the GHL import, which stamps the import day) showed a whole column of
// identical "Week 1" rows. A week she wasn't a client for isn't week 1 of
// anything — callers fall back to the date instead.
export function weekOnProgram(startDate, date = todayInBoise()) {
