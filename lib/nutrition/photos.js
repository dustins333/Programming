...
import { toStoredFraming } from "./photoFraming";
...
// How this photo sits inside its frame (migration 0124). Stored on the
// photo, not on a comparison: what moved between two shots is the camera,
// so a photo nudged once into a standard frame lines up against every other
// adjusted photo rather than only the one beside it today.
//
// An adjustment that lands back where it started is written as NULL, so
// "has anyone touched this?" stays an honest question of the data.
//
// Asks for the row back because an UPDATE that RLS filters out affects zero
// rows and returns NO error -- it would report success, the coach would
// look away, and the framing would spring back on the next load.
export async function updatePhotoFraming(photoId, framing) {
  const { data, error } = await supabase
    .from("photos")
    .update({ framing: toStoredFraming(framing) })
    .eq("id", photoId)
    .select("id");
  if (error) throw error;
  if (!data || data.length === 0) throw new Error("That photo could not be updated.");
}
...
// from the Monday picker on the Settings tab (ClientSettingsPanel),
// written through that panel's own updateClient patch alongside the cadence — so the pair of
