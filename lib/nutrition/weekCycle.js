...
// Where one week's check-in stands. Four outcomes, in strict priority
// order — a real response always wins.
//
// `closeout` (programming.nutrition_checkin_closeouts, migration 0111) is
// the coach saying "this one isn't coming, stop showing it as outstanding".
// It is deliberately checked AFTER the response, not instead of it: closing
// a week out doesn't stop the client submitting, so if she does get to it
// late the submission supersedes the close-out everywhere this is read,
// with nothing to clean up.
//
// The parameter is optional so a caller that has no close-out data behaves
// exactly as it did before this existed.
export function deriveCheckinStatus(response, closeout = null) {
  if (response) return response.finalized_at ? "completed" : "ready";
  if (closeout) return "closed";
  return "pending";
}
