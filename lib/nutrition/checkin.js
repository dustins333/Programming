...
export async function submitCheckin(userId, answers, { photosSkipReason, weekStart } = {}) {
  ...
  const targetWeekStart = weekStart ?? computeWeekWindows(today).currentWeek.start;
  ...
  if (clientRow && isPhotoRequirementWeek(clientRow, targetWeekStart)) {
    ...
    // that week's own start — a today-relative recency window can never be
    // satisfied for a weeks-old week, which used to make the UI's ✓ (which
    // correctly counted from week_start) submit into an unrecoverable error.
    const photoCutoff = weekStart ?? addDays(today, -PHOTO_RECENCY_DAYS);
    const { data: recentPhotos } = await supabase.from("photos").select("angle").eq("client_id", userId).gte("date", photoCutoff);
