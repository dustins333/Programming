...
import { isPhotoRequirementWeek, hasAllAngles, PHOTO_RECENCY_DAYS } from "./photos";
...
// lib/nutrition/photos.js) — same gate the standalone app enforces — unless
// the caller passes photosSkipReason (member explained why they can't this
// week). Deliberately not a new column on the shared checkin_responses
// table (public.*, also used by the standalone Nutrition Tracker app) — the
// skip reason is just appended as one more {question, answer} pair into the
// existing `answers` jsonb array, so this stays fully additive/harmless to
// that app's own rows and rendering.
export async function submitCheckin(userId, answers, { photosSkipReason } = {}) {
  ...
  if (clientRow && isPhotoRequirementWeek(clientRow, currentWeek.start)) {
    const { data: recentPhotos } = await supabase.from("photos").select("angle").eq("client_id", userId).gte("date", addDays(today, -PHOTO_RECENCY_DAYS));
    if (!hasAllAngles(recentPhotos ?? [])) {
      if (!photosSkipReason) {
