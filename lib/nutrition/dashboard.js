...
// "Objective tracking set up" = either the coach hasn't sent onboarding to
// the client yet (migration 0031 — nothing for the client to see or do,
// regardless of how much the coach has already assigned/edited behind the
// scenes) or has sent it but hasn't assigned any tracking days yet.
// "Objective tracking" = sent AND days assigned, client is logging (even if
// fully logged, as long as the questionnaire or starting photos aren't both
// in yet too — readyForReview needs all 3). "Ready for review" = all 3
// onboarding phases done. Checked before the sent/tracking-count gate below
// since a bypassed-onboarding client can reach readyForReview without ever
// being "sent" at all.
function deriveOnboardingBucket(phases, trackingDatesCount, sent) {
  if (phases.readyForReview) return "readyForReview";
  if (!sent || trackingDatesCount === 0) return "otSetup";
  return "otInProgress";
}
...
export async function getNutritionRoster(today = todayInBoise()) {
  ...
  const [
    ...
    getOnboardingPhasesForClients(onboardingIds),
  ]);
  ...
  return clients.map((client) => {
    ...
    const onboardingBucket = onboardingInfo
      ? deriveOnboardingBucket(onboardingInfo.phases, onboardingInfo.trackingDatesCount, Boolean(client.onboarding_sent_at))
      : null;
