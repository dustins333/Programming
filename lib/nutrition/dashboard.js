...
import { listClosedOutUserIdsForWeek } from "./checkin";
import { listUpcomingCheckinBookings } from "./checkinBooking";
...
function deriveRosterStatus({ status, hasTarget, isOnboarding, onboardingBucket, checkinStatus }) {
  ...
  if (checkinStatus === "closed") return "checkinClosed";
...
export async function getNutritionRoster(today = todayInBoise()) {
  ...
  const [
    ...
    // Isolated for the same reason as the onboarding phases below: this
    // reads a table added in 0111, and an unrun migration must not be able
    // to take down a roster that is otherwise fine. Falling back to an
    // empty set means nothing reads as closed out, which is exactly the
    // pre-0111 behaviour.
    listClosedOutUserIdsForWeek(userIds, currentWeek.start).catch((err) => {
      console.error("Nutrition roster: check-in close-outs failed", err);
      return new Set();
    }),
    ...
    // Isolated for the same reason as the two above: this reads a table
    // added in 0113. An empty map means no client shows a call, which is
    // exactly the pre-0113 behaviour -- never a blank roster.
    listUpcomingCheckinBookings(userIds, today).catch((err) => {
      console.error("Nutrition roster: check-in bookings failed", err);
      return new Map();
    }),
  ]);
  ...
  return clients.map((client) => {
    ...
    const checkinStatus = deriveCheckinStatus(checkin, closedOutUserIds.has(client.id) ? { user_id: client.id } : null);
    ...
    return {
      ...
      // The next check-in call on the calendar, or null. What tells
      // "waiting on her" apart from "we have a call booked" on the roster.
      checkinBooking: bookingByUserId.get(client.id) ?? null,
      needsAttention,
