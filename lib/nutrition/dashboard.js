...
// "Objective tracking set up" = the coach hasn't assigned any tracking days
// yet (nothing for the client to do). "Objective tracking" = days assigned,
// client is logging (even if fully logged, as long as the questionnaire or
// starting photos aren't both in yet too — readyForReview needs all 3).
// "Ready for review" = all 3 onboarding phases done.
function deriveOnboardingBucket(phases, trackingDatesCount) {
  if (phases.readyForReview) return "readyForReview";
  if (trackingDatesCount === 0) return "otSetup";
  return "otInProgress";
}
...
export async function getNutritionRoster(today = todayInBoise()) {
  ...
  return clients.map((client) => {
    ...
    const onboardingBucket = onboardingInfo
      ? deriveOnboardingBucket(onboardingInfo.phases, onboardingInfo.trackingDatesCount)
      : null;
