...
// separately via core.users.first_login_at (migration 0021) rather than
// folded into this 3-phase stepper, since "signed in at all" is an
// account-level fact, not a nutrition-onboarding-specific one — see
// getOnboardingRoster below, which is what actually surfaces it to coaches.
...
// Ported from the standalone app's lib/clientStatus.js's computeClientStatus
// — one human-readable line + a done/neutral tone for the coach's
// Onboarding list and the client detail page's status pill. Takes the same
// `phases` shape computeOnboardingPhases already produces rather than
// re-deriving from raw booleans, since every call site here already has
// that object on hand.
export function describeOnboardingProgress(phases) {
  if (!phases.questionnaire) return { text: "Waiting on questionnaire", tone: "neutral" };
  if (!phases.tracking) return { text: "Waiting on Objective Tracking days", tone: "neutral" };
  if (!phases.photos) return { text: "Waiting on starting photos", tone: "neutral" };
  return { text: "Ready for review", tone: "done" };
}
...
// Ported from the standalone app's lib/onboardingClients.js's
// getOnboardingClients() — every not-yet-approved client with their
// progress status attached, batched (fixed number of queries, not one per
// client) for the coach's Onboarding list page. Cross-schema (core.users
// for first_login_at) fetched separately and merged client-side rather than
// a PostgREST embed, same rule as everywhere else in this app (see
// CLAUDE.md's "Cross-schema PostgREST embeds are avoided").
export async function getOnboardingRoster() {
  const { data: clients, error: clientsError } = await supabase
    .from("clients")
    .select("*")
    .is("objective_tracking_approved_at", null)
    .order("name");
  ...
  const [
    ...
    // Reads auth.users.last_sign_in_at via a staff-only security-definer
    // function (migration 0022) — see AuthProvider.js's comment for why
    // this isn't a custom-stamped core.users column: a new column has no
    // history, auth.users' native tracking does.
    core.rpc("get_login_activity", { user_ids: userIds }),
  ]);
  ...
  const lastSignInById = Object.fromEntries((loginRows ?? []).map((u) => [u.id, u.last_sign_in_at]));

  return clients.map((client) => {
    const dates = datesByClient[client.id] ?? [];
    const logged = loggedByClient[client.id] ?? new Set();
    const trackingComplete = dates.length > 0 && dates.every((d) => logged.has(d));
    const phases = computeOnboardingPhases(respondedIds.has(client.id), trackingComplete, hasAllAngles(photosByClient[client.id] ?? []));

    return {
      ...client,
      firstLoginAt: lastSignInById[client.id] ?? null,
      phases,
      progress: describeOnboardingProgress(phases),
    };
  });
}
