...
import { daysBetween, mondayOnOrBefore } from "../boiseDate";
...
// this list.
// Short on purpose: these read down the card's left edge a letter at a
// time (PhaseRail), so a long name costs height there. "Maintenance" was
// renamed to "Maint" in the data for that reason.
export const SUGGESTED_PHASES = ["Diet", "Reverse", "Maint", "Build"];
...
// The phase covering a plain DATE rather than a week start. Progress photos
// carry a date, so the phase they were taken in is whatever marker covers
// the Monday-Sunday week containing them — which is the same week the Weeks
// tab draws that photo's day inside, so a coach reading "Diet 3" on a photo
// and "Diet 3" down the side of a week is reading one fact.
//
// Every week_start in the table is a Monday (the 0111 CHECK), so snapping
// BACKWARD is what lands on the containing week. Not mondayOnOrAfter, which
// weekCycle.js uses for the photo-cadence anchor and would push a Tuesday
// photo into the following week.
export function phaseForDate(markers, date) {
  if (!date) return null;
  return resolveWeekPhase(markers, mondayOnOrBefore(date));
}

// "Diet 3" — the phase and how many weeks into it, worded exactly as the
// spine on the Weeks tab reads it.
export function phaseLabelForDate(markers, date) {
  const phase = phaseForDate(markers, date);
  return phase ? `${phase.name} ${phase.number}` : null;
}
