...
// two signals Safari's classifier keys on).
//
// The name must contain NO dash. A dash in a name attribute is its own
// Safari heuristic — it classifies that field as a phone number, shows the
// contacts dropdown, and ignores autocomplete="off" — which is exactly the
// "it thinks you want a phone number" symptom that survived the first
// attempt at this, when the generated name was "kova-search-1". Dashes are
// only safe in the specific form "-<digit>-"; not worth relying on when a
// name with no dash at all does the job.
//
// Both signals are safe here —
...
export function suppressSafariAutofill(node) {
  ...
  try {
    ...
    // No dash — see above. Leads with "search" so the classifier's own
    // regex has the least possible room to miss it.
    if (!node.name) node.name = `searchfield${++nameCounter}`;
  } catch {
