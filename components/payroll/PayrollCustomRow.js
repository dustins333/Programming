...
export function PayrollCustomRow({ custom, onPress }) {
  ...
  return (
    <PayrollTile checkState={checkState} onCheckPress={onPress} onPress={onPress} height={80}>
      <View className="flex-1 flex-row items-center justify-between">
        <View className="flex-1 pr-3">
          ...
          <Text numberOfLines={1} style={{ fontFamily: fonts.sansSemiBold, fontSize: 14, color: "#44403c" }}>
            {custom ? custom.custom_description || "Custom amount" : "Tap to add a custom payroll request"}
          </Text>
        </View>
