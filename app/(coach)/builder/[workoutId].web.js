...
function SortableExerciseRow({ item, onChange, onRemove, onLinkSuperset, onUnlinkSuperset, isLastRow, linkedToNext }) {
  ...
  // 2px warm border on every lift (not the old hairline stone-200) so each
  // one reads as its own card at a glance; full terracotta + a peach fill
  // for anything in a superset.
