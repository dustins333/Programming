...
**Status as of 2026-08-19: Phase 1 built and live.** Every rule below was decided
by Terra in conversation and validated against 22 months of real data. A build
session should not need to re-ask any of it. Open items are listed at the end.

Phase 1 shipped: `programming.ccrew_*` (migration 0067), the coach upload/preview/
commit screen, the CCrew view screen, and the 22-month backfill (migration 0068,
generated by `scripts/ccrew_backfill.mjs`). Phases 2 and 3 are untouched, and the
member-facing display is still open. Re-run `scripts/ccrew_validate.mjs` after any
change to `lib/ccrew/rules.js` — it replays July 2026 and re-checks the backfill.
...
**Conditioning stays in this table, in the max — weighed and left alone
(2026-08-19).** The alternative considered was treating it as an add-on that
stacks (`Group Strength 3x` + `Conditioning` = 4 a week, 16 a month). Rejected
on evidence: Kilo folds conditioning sessions into `Total Attendance` — Cristin
Ellis holds only `Conditioning` and shows 4 class sessions — and nothing in the
export distinguishes a conditioning session from a strength one, so "count the
attendance but not the requirement" is not implementable. Making it raise the
bar would have penalised anyone with a lapsed Conditioning signup still on
their Kilo record, which this data demonstrably has. Being in the max means it
never raises anyone's bar (every real commitment is >= 1) while
conditioning-only still resolves to 1, i.e. ineligible. Applied to July 2026 it
would have dropped Angie Adams (10 sessions) and Meredith Wallis (11) off the
wall. Conditioning sessions therefore count as bonus attendance toward whatever
the member's package already commits them to.
...
- Current-streak distribution: 33 at 0 (of those with history), 18 at 1, 26 at 2,
  ...
  18 people with no history at all.

If a build run produces materially different numbers, something is wrong.

**Corrected 2026-08-19 during the Phase 1 build.** This block previously read
123 with history, 39 at 0, and quoted Callie White at 19 lifetime months and
Danielle Hinkson at 18. Those figures were computed with the 2024 tabs taken at
**face value** — the one thing the Backfill section above says not to do — so
they are inconsistent with the rules, not with the implementation. Specifically:

- Face value credits **127** people with history. The spec subtracted 4, but
  applying the documented rules removes **6**: Amanda Rynes (best month 0.38),
  Barbara Wright (0.25), Liz Marsden (0.75) and Dustin Smout (0 attendance) —
  the four meant — plus **Cristin Ellis** and **Mary Ann Allison**, who both
  resolve to a 1x target and are caught by rule 3's `>= 2` eligibility gate.
  Rule 3 names Cristin Ellis explicitly as someone who can never make CCrew, so
  excluding her is deliberate, not a regression.
- Callie White and Danielle Hinkson each have exactly one 2024 month below the
  bar (Callie: 9 sessions on a 3x package = 0.75, which is the precise example
  rule 4 gives as failing; Danielle: 8 on a 3x = 0.667), so 18 and 17 are
  correct under the rules and 19/18 are the face-value figures.

Lisa Allen's 21 is unaffected and matches exactly — she genuinely passed all
three 2024 months. The rules, not these numbers, are the authority; the
validation script asserts the corrected figures and explains why in a comment.
